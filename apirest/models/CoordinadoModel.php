<?php
class CoordinadoModel extends CI_Model{
    ...
    public function getReporte($arrParams){
		...
		if(( $this->user->ID_Usuario == 1 || $this->user->ID_Grupo == 1503 ) && $arrParams['ID_Filtro_Empresa']==0){
			$where_id_empresa = "";
		} else if(( $this->user->ID_Usuario == 1 || $this->user->ID_Grupo == 1503 ) && $arrParams['ID_Filtro_Empresa']!=0){
...
WHERE
Nu_Servicio_Transportadora_Dropshipping = 2
...
PC.ID_Pedido_Cabecera = " . $ID;
    ...
    public function generarGuia99($ID_Pedido_Cabecera){
		...
		if ($err) {
			...
			PC.Txt_Direccion_Entidad_Order_Address_Entry AS direccion_ref_destination,
