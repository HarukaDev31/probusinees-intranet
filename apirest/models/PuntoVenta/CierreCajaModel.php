...
AND VC.ID_Matricula_Empleado = " . $iIdMatriculaPersonal . "
...
AND AC.ID_Almacen = " . $this->session->userdata['almacen']->ID_Almacen."
AND AC.ID_Matricula_Empleado = " . $iIdMatriculaPersonal . "
AND TOC.Nu_Tipo!=4
...
MONE.No_Signo,
TOC.Nu_Tipo";
		//campo Nu_Tipo - tabla tipo_operacion_caja
		//3=apertura de caja, 4=cierre de caja , 5 = ingreso de dinero, 6 = salida de dinero
		if ( !$this->db->simple_query($query) ){
			$error = $this->db->error();
			return array(
				'sStatus' => 'danger',
...
VC.ID_Documento_Cabecera";
		...
		//campo Nu_Tipo = medio_pago
		//0=efectivo, 1=credito y 2=tarjeta o deposito
		//Nu_Tipo_Caja = = medio_pago
		//0=fisico y 1 = virtual
...
MP.Nu_Tipo_Caja";
    ...
    public function addCierreCaja($arrData){
		...
		if ( isset($this->session->userdata['arrDataPersonal']) )  {
			$arrValidacionCajaCerrada = $this->validacionCajaCerrada();
			$iStatusAperturaCaja = $arrValidacionCajaCerrada['result']->Nu_Tipo;
		}
		
		if ( $iStatusAperturaCaja=='3' ) {//3=Si la caja esta aperturada y tiene session abierta
			...
			$arrData = array(
				...
				'ID_Tipo_Operacion_Caja_Apertura' => $arrData['iIdTipoOperacionCajaApertura'],
				'ID_Tipo_Operacion_Caja' => $arrData['iIdTipoOperacionCaja'],
				'ID_Moneda' => $this->session->arrDataPersonal['arrData']['ID_Moneda'],
				'Ss_Expectativa' => $arrData['fTotalLiquidar'],
				'Ss_Total' => $arrData['fTotalDepositado'],
				'Txt_Nota' => $arrData['sNotaCaja'],
				'Nu_Estado' => 1,
			);
...
LIMIT 1";
			...
			if ( isset($this->session->userdata['arrDataPersonal']) )  {
				...
				return array(
					'sStatus' => 'success',
					'sMessage' => 'La caja ya está cerrada',
					'iIdMatriculaEmpleado' => $arrValidacionCajaCerrada['result']->ID_Matricula_Empleado,
					'iIdEnlaceAperturaCaja' => $arrValidacionCajaCerrada['result']->ID_Enlace_Apertura_Caja_Pos,
					'iIdEnlaceCierreCaja' => $arrValidacionCajaCerrada['result']->ID_Caja_Pos
				);
			} else {				
				return array(
					'sStatus' => 'success',
					'sMessage' => 'La caja ya está cerrada',
					'iIdMatriculaEmpleado' => '',
					'iIdEnlaceAperturaCaja' => '',
					'iIdEnlaceCierreCaja' => ''
				);
			}
		}
	...
	public function validacionCajaCerrada(){
		$query = "SELECT
CJ.ID_Caja_Pos,
CJ.ID_Matricula_Empleado,
CJ.ID_Enlace_Apertura_Caja_Pos,
TOC.Nu_Tipo
FROM
caja_pos AS CJ
JOIN tipo_operacion_caja AS TOC ON(CJ.ID_Tipo_Operacion_Caja=TOC.ID_Tipo_Operacion_Caja)
WHERE
CJ.ID_POS = " . $_SESSION['arrDataPersonal']['arrData'][0]->ID_POS . "
AND CJ.ID_Matricula_Empleado = " . $_SESSION['arrDataPersonal']['arrData'][0]->ID_Matricula_Empleado . "
AND TOC.Nu_Tipo IN(3,4)
ORDER BY
CJ.Fe_Movimiento DESC LIMIT 1;";
