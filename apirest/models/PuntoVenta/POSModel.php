...
FROM
serie_documento
WHERE ID_Tipo_Documento=" . $arrPost['arrCabecera']['ID_Tipo_Documento'] . "
AND Nu_Estado=1
AND ID_POS=".$this->session->userdata['arrDataPersonal']['arrData'][0]->ID_POS." LIMIT 1";
		$arrSerieDocumento = $this->db->query($query_numero)->row();
		...
		if ($this->db->query("SELECT COUNT(*) AS existe FROM documento_cabecera WHERE ID_Empresa = " . $this->user->ID_Empresa . " AND ID_Tipo_Asiento = 1 AND ID_Tipo_Documento = " . $arrPost['arrCabecera']['ID_Tipo_Documento'] . " AND ID_Serie_Documento = '" . $arrSerieDocumento->ID_Serie_Documento . "' AND ID_Numero_Documento = '" . $arrSerieDocumento->Nu_Numero_Documento . "' LIMIT 1")->row()->existe > 0){
			...
			if ( !empty($Last_ID_Entidad) ){
				...
				if ((!empty($arrPost['arrCliente']['ID_Departamento']) && $arrClienteBD[0]->ID_Departamento != $arrPost['arrCliente']['ID_Departamento'])){
					$sql = "UPDATE entidad SET ID_Departamento = " . $arrPost['arrCliente']['ID_Departamento'] . " WHERE ID_Entidad = " . $Last_ID_Entidad;
					$this->db->query($sql);
				}

				if ((!empty($arrPost['arrCliente']['ID_Provincia']) && $arrClienteBD[0]->ID_Provincia != $arrPost['arrCliente']['ID_Provincia'])){
					$sql = "UPDATE entidad SET ID_Provincia = " . $arrPost['arrCliente']['ID_Provincia'] . " WHERE ID_Entidad = " . $Last_ID_Entidad;
					$this->db->query($sql);
				}

				if ((!empty($arrPost['arrCliente']['ID_Distrito']) && $arrClienteBD[0]->ID_Distrito != $arrPost['arrCliente']['ID_Distrito'])){
					$sql = "UPDATE entidad SET ID_Distrito = " . $arrPost['arrCliente']['ID_Distrito'] . " WHERE ID_Entidad = " . $Last_ID_Entidad;
					$this->db->query($sql);
				}
			...
			if (
				...
				if ( $arrResponseSQL->num_rows() > 0 ){
					...
					if ( !empty($arrPost['arrCliente']['ID_Departamento']) ){
						$arrCliente = array_merge($arrCliente, array("ID_Departamento" => $arrPost['arrCliente']['ID_Departamento']));
					}
					if ( !empty($arrPost['arrCliente']['ID_Provincia']) ){
						$arrCliente = array_merge($arrCliente, array("ID_Provincia" => $arrPost['arrCliente']['ID_Provincia']));
					}
					if ( !empty($arrPost['arrCliente']['ID_Distrito']) ){
						$arrCliente = array_merge($arrCliente, array("ID_Distrito" => $arrPost['arrCliente']['ID_Distrito']));
					}
					if ($this->db->insert('entidad', $arrCliente) > 0) {
						$Last_ID_Entidad = $this->db->insert_id();
					} else {
						$this->db->trans_rollback();
						return array('sStatus' => 'warning', 'sMessage' => 'No se puede registrar Cliente');
					}
				}
			}// ./ if cliente nuevo
			...
			if(empty($Last_ID_Entidad)) {
				$this->db->trans_rollback();
				return array('sStatus' => 'warning', 'sMessage' => 'Sin cliente');
			}

			//validar clientes varios a factura
			if ($this->empresa->ID_Entidad_Clientes_Varios_Venta_Predeterminado == $Last_ID_Entidad && $arrPost['arrCabecera']['ID_Tipo_Documento'] == 3) {
				$this->db->trans_rollback();
				return array('sStatus' => 'warning', 'sMessage' => 'No se puede emitir FACTURA a un CLIENTES VARIOS');
			}
			
			if ( $arrPost['arrCabecera']['ID_Sunat_Tipo_Transaction'] == 1 && $arrPost['arrCabecera']['ID_Tipo_Documento'] == '3' && $arrPost['arrCliente']['ID_Tipo_Documento_Identidad'] == 1 ) {//1=OTROS
				$this->db->trans_rollback();
				return array('sStatus' => 'warning', 'sMessage' => 'Para ventas con Tipo operación VENTA INTERNA no puedes FACTURAR a T.D.I. = OTROS');
			}

			if ( $arrPost['arrCabecera']['ID_Sunat_Tipo_Transaction'] == 1 && $arrPost['arrCabecera']['ID_Tipo_Documento'] == '3' && $arrPost['arrCliente']['ID_Tipo_Documento_Identidad'] == 2 ) {//2=DNI
				$this->db->trans_rollback();
				return array('sStatus' => 'warning', 'sMessage' => 'No se puede FACTURAR a un cliente con DNI');
			}

			if ( $arrPost['arrCabecera']['ID_Sunat_Tipo_Transaction'] == 2 && $arrPost['arrCabecera']['ID_Tipo_Documento'] == '3' && $arrPost['arrCliente']['ID_Tipo_Documento_Identidad'] != 1 ) {
				$this->db->trans_rollback();
				return array('sStatus' => 'warning', 'sMessage' => 'Para ventas con Tipo operación EXPORTACIÓN seleccionar T.D.I. = OTROS');
			}

			if ( $arrPost['arrCabecera']['ID_Sunat_Tipo_Transaction'] == 1 && $arrPost['arrCabecera']['ID_Tipo_Documento'] == '3' && strlen(trim($arrPost['arrCliente']['Nu_Documento_Identidad'])) < 10 ) {
				$this->db->trans_rollback();
				return array('sStatus' => 'warning', 'sMessage' => 'RUC debe de tener mínimo 11 caracteres');
			}
...
1
			...
			$arrVentaCabecera = array(
				...
				'ID_Mesero' => ($this->empresa->Nu_Tipo_Vender_Usuario_POS == 1 ? '' : $this->session->usuario->ID_Usuario)
			);

			if ( !empty($arrPost['arrCliente']['ID_Distrito']) )
				$arrVentaCabecera = array_merge($arrVentaCabecera, array("ID_Distrito_Delivery" => $this->security->xss_clean($arrPost['arrCliente']['ID_Distrito'])));
			...
			foreach($arrPost['arrDetalle'] as $row) {
				...
				if ($row['iRegaloInafectoBonificacion']==1) {
					...
					//set regalo para api sunat
					$row['tipo_impuesto_sunat'] = 4;//GRATUITA
					$row['fImpuestoConfigurado'] = 1;
					$row['codigo_impuesto_sunat'] = '31';
					$row['codigo_impuesto_sunat_pse'] = '10';
				}
				...
				//crear array para api de sunat
				$arrVentaDetalleSunat['detalle'][] = array(
					"codigo_unidad_medida_sunat" => $row['codigo_unidad_medida_sunat'],
					"codigo_item" => $row['sCodigoItem'],
					"nombre_item" => $row['nombre_item'],
					"nota" => $row['Txt_Nota'],
					"cantidad" => $Qt_Producto,
					"tipo_impuesto_sunat" => $row['tipo_impuesto_sunat'],
					"importe_impuesto_sunat" => substr($row['fImpuestoConfigurado'],0,-2),
					"codigo_impuesto_sunat" => $row['codigo_impuesto_sunat'],
					"codigo_impuesto_sunat_pse" => $row['codigo_impuesto_sunat_pse'],
					"precio" => $row['Ss_Precio'],
					"impuesto" => $fImpuestoItem,
					"subtotal" => $fSubTotalItem,
					"descuento_subtotal" => $row['fDescuentoSinImpuestosItem'],
					"total" => $row['Ss_Total_Producto'],
				);
			}
			...
			$sCadenaTextoMedioPagoSunat = '';
			$arrVentasCreditoCuotasSunat = array();
			foreach($arrPost['arrFormaPago'] as $row) {
				//monto que realmente entra a caja
				$fTotalMedioPago = $this->security->xss_clean($row['Ss_Total']);
				if($row['iTipoVista'] == '1' && $fVuelto > 0.00) {
					$fTotalMedioPago = $arrPost['arrCabecera']['Ss_Total'] - $fTotalGratuita;
				...
				//datos para api SUNAT
				if($row['iTipoVista'] == '1') {
					$arrVentasCreditoCuotasSunat['venta_al_credito'] = array(
						'venta_al_credito' => array(
							0 => array(
								'cuota' => 1,
								'fecha_de_pago' => $arrVentaCabecera['Fe_Vencimiento'],
								'importe' => $arrVentaCabecera['Ss_Total_Saldo']
							)
						)
					);
				}
				...
				$sCadenaTextoMedioPagoSunat .= $row['nombre_medio_pago'] . ' [' . $row['texto_medio_pago'] . ']: ' . ($row['iTipoVista'] != '1' ? $this->session->userdata['arrDataPersonal']['arrData']['No_Signo'] . ' ' . $fTotalMedioPago : $fTotalMedioPago) . ', ';
			}
			...
			if ($this->db->trans_status() === FALSE) {
				...
				if ( $this->empresa->Nu_Estado_Pago_Sistema == 1 && $arrPost['arrCabecera']['ID_Tipo_Documento'] != '2') {// cancelado y 2 = Documento interno
					...
					if ( $this->empresa->Nu_Tipo_Proveedor_FE == 1 ) {//Nubefact
						...
						//versión nueva para envío de SUNAT 2.0
						$arrPostFeSunatDetalle = array(

						);

						//SUNAT TIPO TRANSACTION
						$sCodigoTipoOperacionSunat = "0101";//VENTA INTERNA
						if ($arrVentaCabecera['ID_Sunat_Tipo_Transaction']=='2')//EXPORTACION
							$sCodigoTipoOperacionSunat = "0200";//EXPORTACION

						$arrPostFeSunatCabecera = array(
							"tipo_operacion_envio" => "generar_comprobante",
							"tipo_conexion_sistema" => $this->empresa->Nu_Tipo_Proveedor_FE,
							"tipo_operacion_sunat" => $sCodigoTipoOperacionSunat,
							"tipo_rubro_empresa" => $this->empresa->Nu_Tipo_Rubro_Empresa,
							"codigo_tipo_documento_sunat" => $arrPost['arrCabecera']['codigo_tipo_documento_sunat'],
							"id_serie_documento" => $arrVentaCabecera['ID_Serie_Documento'],
							"cantidad_caracteres_correlativo" => $arrSerieDocumento->Nu_Cantidad_Caracteres,
							"id_numero_documento" => $arrVentaCabecera['ID_Numero_Documento'],
							"Codigo_Tipo_Documento_Identidad_Sunat" => $arrPost['arrCliente']['codigo_tipo_documento_identidad_sunat'],
							"Nu_Documento_Identidad" => $arrPost['arrCliente']['Nu_Documento_Identidad'],
							"No_Entidad" => $arrPost['arrCliente']['No_Entidad'],
							"direccion_delivery" => $arrPost['arrCabecera']['sDireccionDelivery'],
							"celular_cliente" => $arrPost['arrCliente']['Nu_Celular_Entidad'],
							"fecha_emision" => $arrVentaCabecera['Fe_Emision'],
							"fecha_hora_emision" => $arrVentaCabecera['Fe_Emision_Hora'],
							"fecha_vencimiento" => $arrVentaCabecera['Fe_Vencimiento'],
							"codigo_moneda_sunat" => $arrPost['arrCabecera']['codigo_moneda_sunat'],
							"nombre_moneda" => $arrPost['arrCabecera']['nombre_moneda'],
							"orden_compra_servicio" => $arrVentaCabecera['No_Orden_Compra_FE'],
							"placa" => $arrVentaCabecera['No_Placa_FE'],
							"formato_pdf" => $arrVentaCabecera['No_Formato_PDF'],
							"tipo_recepcion" => $arrVentaCabecera['Nu_Tipo_Recepcion'],
							"retencion" => $arrVentaCabecera['Nu_Retencion'],
							"retencion_total" => $arrVentaCabecera['Ss_Retencion'],
							"detraccion" => $arrVentaCabecera['Nu_Detraccion'],
							"detraccion_total" => $arrVentaCabecera['Ss_Detraccion'],
							"glosa" => $arrVentaCabecera['Txt_Glosa'],
							"total_saldo" => $arrVentaCabecera['Ss_Total_Saldo'],
							"total" => $arrVentaCabecera['Ss_Total'],
							"vuelto" => $arrVentaCabecera['Ss_Vuelto'],
							"descuento_subtotal" => $arrVentaCabecera['Ss_Descuento'],
							"guias" => strtoupper($arrPost['arrCabecera']['Txt_Garantia']),
							"nombre_vendedor" => $_SESSION['arrDataPersonal']['arrData'][0]->No_Entidad,
							"hash_cdr" => "",
							"id_documento_cabecera" => $Last_ID_Documento_Cabecera,
							"id_venta" => $Last_ID_Documento_Cabecera,
							"estado_venta" => 6,
							"id_almacen" => $arrVentaCabecera['ID_Almacen'],
							"id_entidad" => $arrVentaCabecera['ID_Entidad'],
							"cadena_texto_medio_pago_sunat" => $sCadenaTextoMedioPagoSunat
						);

						//array ventas al credito
						$arrPostFeSunat = array_merge($arrPostFeSunatCabecera, $arrVentasCreditoCuotasSunat, $arrVentaDetalleSunat);
						$arrResponseFE = $this->ComprobanteElectronicoSunatModel->generarFormatoVentaElectronicoSunat( $arrPostFeSunat );
						if ( $arrResponseFE['status'] != 'success' ) {
							$arrResponseFE['sStatus'] = $arrResponseFE['status'];
							$arrResponseFE['sMessage'] = $arrResponseFE['message'];
							return $arrResponseFE;
						}
						$arrResponseFE['enlace_del_pdf'] = $arrResponseFE['data']['Txt_Url_PDF'];
						// fin versión 2.0

						/*
						//versión antigua para envío de SUNAT 1.0
