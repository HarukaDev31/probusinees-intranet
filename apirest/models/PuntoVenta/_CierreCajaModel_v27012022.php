...
class CierreCajaModel extends CI_Model{	
    ...
    public function obtenerVentasMultiples($arrParams){
		...
		$query = "SELECT
" . $campo_familia_item . "
SUM((CASE WHEN VC.ID_Tipo_Documento!=5 THEN VD.Qt_Producto ELSE 0 END) - (CASE WHEN VC.ID_Tipo_Documento=5 THEN VD.Qt_Producto ELSE 0 END)) AS Qt_Producto,
MONE.No_Signo,
SUM((CASE WHEN VC.ID_Tipo_Documento!=5 THEN VD.Ss_Total ELSE 0 END) - (CASE WHEN VC.ID_Tipo_Documento=5 THEN VD.Ss_Total ELSE 0 END)) AS Ss_Total
FROM
...
VC.ID_Documento_Cabecera";
		...
		if ( $arrResponseSQL->num_rows() > 0 ){
			...
			foreach ($arrResponseSQL->result() as $row){
				...
				/*
				if($row->Ss_Total_Saldo==0.00 && $row->Nu_Tipo == 0) {
					...
					$fTotalxDocumento += ($row->Ss_Total_VMP - $row->Ss_Total_VMP_NC);
					if ( $row->Nu_Tipo_Caja != 0 )
						$fTotalOtrosMPxDocumento += ($row->Ss_Total_VMP - $row->Ss_Total_VMP_NC);
					if ( $row->Nu_Tipo_Caja == 0 )
						$fTotalEfectivoxDocumento += ($row->Ss_Total_VMP - $row->Ss_Total_VMP_NC);
					if ( $fTotalxDocumento > ($row->Ss_Total_VC - $row->Ss_Total_VC_NC) )
						$fSumTotalEfectivo += ($row->Ss_Total_VC - $row->Ss_Total_VC_NC) - $fTotalOtrosMPxDocumento;
					else if ( $fTotalxDocumento == ($row->Ss_Total_VC - $row->Ss_Total_VC_NC) )
