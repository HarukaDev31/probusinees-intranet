<?php
class GuiaSalidaModel extends CI_Model{
    ...
    var $column_order = array('VC.Fe_Emision', 'VC.ID_Tipo_Documento', 'VC.ID_Serie_Documento', 'VC.ID_Numero_Documento', 'TDOCUIDEN.No_Tipo_Documento_Identidad_Breve', 'CLI.No_Entidad', 'MONE.No_Signo', null, null);
	...
	public function _get_datatables_query(){
        ...
        $this->db->select('VC.ID_Guia_Cabecera, VE.ID_Documento_Cabecera, VC.Fe_Emision, TDOCU.No_Tipo_Documento_Breve, VC.ID_Serie_Documento, VC.ID_Numero_Documento, TDOCUIDEN.No_Tipo_Documento_Identidad_Breve, CLI.No_Entidad, MONE.No_Signo, ROUND(VC.Ss_Total, 2) AS Ss_Total, VC.Nu_Descargar_Inventario, VC.Nu_Estado, TDESTADO.No_Class AS No_Class_Estado, TDESTADO.No_Descripcion AS No_Descripcion_Estado')
    ...
    public function get_by_id($ID, $Nu_Tipo_Operacion){
        if ($Nu_Tipo_Operacion === '0') {//Factura y Guia
            ...
            SELECT
            	...
            	VC.Po_Descuento,
				VE.Nu_Tipo_Motivo_Traslado,
				VE.Fe_Traslado,
				VE.ID_Entidad_Transportista,
				VE.No_Chofer,
				VE.Nu_Licencia,
				VE.No_Placa,
				VE.Txt_Certificado_Inscripcion,
				VE.Txt_Direccion_Llegada,
				VE.Txt_Referencia_Direccion_Llegada
            FROM
            	documento_cabecera AS VC
            	JOIN documento_detalle AS VD
            		ON (VC.ID_Documento_Cabecera = VD.ID_Documento_Cabecera)
            	JOIN " . $this->table_entidad . " AS CLI
            		ON (CLI.ID_Entidad = VC.ID_Entidad AND CLI.Nu_Tipo_Entidad = 1)
        		...
        		SELECT
					...
					FLE.Nu_Tipo_Motivo_Traslado,
					FLE.Fe_Traslado,
					FLE.ID_Entidad AS ID_Entidad_Transportista,
					FLE.No_Chofer,
					FLE.Nu_Licencia,
					FLE.No_Placa,
					FLE.Txt_Certificado_Inscripcion,
					GC.Txt_Direccion_Llegada,
					GC.Txt_Referencia_Direccion_Llegada
        		FROM
        			documento_cabecera AS VC
	            	...
	            	JOIN flete AS FLE
	            		ON (FLE.ID_Guia_Cabecera = GC.ID_Guia_Cabecera)
                WHERE
            ...
            SELECT
				...
				FLE.No_Placa,
				FLE.Txt_Certificado_Inscripcion,
				VC.Txt_Direccion_Llegada,
				VC.Txt_Referencia_Direccion_Llegada
            FROM
            	guia_cabecera AS VC
            	JOIN guia_detalle AS VD
            		ON (VC.ID_Guia_Cabecera = VD.ID_Guia_Cabecera)
            	JOIN flete AS FLE
            		ON (FLE.ID_Guia_Cabecera = VC.ID_Guia_Cabecera)
            	JOIN " . $this->table_entidad . " AS CLI
            		ON (CLI.ID_Entidad = VC.ID_Entidad AND CLI.Nu_Tipo_Entidad = 1)
            	JOIN producto AS PRO
	        		ON (PRO.ID_Producto = VD.ID_Producto)
    ...
    public function agregarGuiaSalida($arrGuiaSalidaCabecera, $arrGuiaSalidaDetalle, $arrFlete){
    	$response = array('status' => 'error', 'style_modal' => 'modal-danger', 'message' => 'Error al insertar');
		if (
            $this->db->query("SELECT COUNT(*) existe FROM documento_cabecera WHERE ID_Empresa = " . $this->user->ID_Empresa . " AND ID_Tipo_Documento = " . $arrGuiaSalidaCabecera['ID_Tipo_Documento_Factura'] . " AND ID_Serie_Documento = '" . $arrGuiaSalidaCabecera['ID_Serie_Documento_Factura'] . "' AND ID_Numero_Documento = '" . $arrGuiaSalidaCabecera['ID_Numero_Documento_Factura'] . "' LIMIT 1")->row()->existe > 0
            ||
            $this->db->query("SELECT COUNT(*) existe FROM " . $this->table . " WHERE ID_Empresa = " . $this->user->ID_Empresa . " AND ID_Tipo_Documento = " . $arrGuiaSalidaCabecera['ID_Tipo_Documento_Guia'] . " AND ID_Serie_Documento = '" . $arrGuiaSalidaCabecera['ID_Serie_Documento_Guia'] . "' AND ID_Numero_Documento = '" . $arrGuiaSalidaCabecera['ID_Numero_Documento_Guia'] . "' LIMIT 1")->row()->existe > 0
            ){
			$response = array('status' => 'warning', 'style_modal' => 'modal-warning', 'message' => 'El registro ya existe');
		} else {
			$this->db->trans_begin();
			
			$Nu_Tipo_Operacion = $arrGuiaSalidaCabecera['ID_Tipo_Operacion'];			    
			unset($arrGuiaSalidaCabecera['ID_Tipo_Operacion']);
		
		    $ID_Tipo_Asiento_Factura = $arrGuiaSalidaCabecera['ID_Tipo_Asiento_Factura'];
		    $ID_Tipo_Documento_Factura = $arrGuiaSalidaCabecera['ID_Tipo_Documento_Factura'];
		    $ID_Serie_Documento_Factura = $arrGuiaSalidaCabecera['ID_Serie_Documento_Factura'];
		    $ID_Numero_Documento_Factura = $arrGuiaSalidaCabecera['ID_Numero_Documento_Factura'];
		    $Ss_Descuento = $arrGuiaSalidaCabecera['Ss_Descuento'];
		    $Po_Descuento = $arrGuiaSalidaCabecera['Po_Descuento'];
		    
		    unset($arrGuiaSalidaCabecera['ID_Tipo_Asiento_Factura']);
		    unset($arrGuiaSalidaCabecera['ID_Tipo_Documento_Factura']);
		    unset($arrGuiaSalidaCabecera['ID_Serie_Documento_Factura']);
		    unset($arrGuiaSalidaCabecera['ID_Numero_Documento_Factura']);
		    unset($arrGuiaSalidaCabecera['Ss_Descuento']);
		    unset($arrGuiaSalidaCabecera['Po_Descuento']);
			
			if ($Nu_Tipo_Operacion === '7' || $Nu_Tipo_Operacion === '0') {//Guia de Remision Venta
			    $arrGuiaSalidaCabecera['ID_Tipo_Asiento'] = $arrGuiaSalidaCabecera['ID_Tipo_Asiento_Guia'];
			    $arrGuiaSalidaCabecera['ID_Tipo_Documento'] = $arrGuiaSalidaCabecera['ID_Tipo_Documento_Guia'];
			    $arrGuiaSalidaCabecera['ID_Serie_Documento'] = $arrGuiaSalidaCabecera['ID_Serie_Documento_Guia'];
			    $arrGuiaSalidaCabecera['ID_Numero_Documento'] = $arrGuiaSalidaCabecera['ID_Numero_Documento_Guia'];
			    
			    unset($arrGuiaSalidaCabecera['ID_Tipo_Asiento_Guia']);
			    unset($arrGuiaSalidaCabecera['ID_Tipo_Documento_Guia']);
			    unset($arrGuiaSalidaCabecera['ID_Serie_Documento_Guia']);
			    unset($arrGuiaSalidaCabecera['ID_Numero_Documento_Guia']);
			    
    			$this->db->insert($this->table, $arrGuiaSalidaCabecera);
    			$Last_ID_Guia_Cabecera = $this->db->insert_id();
                
    			foreach ($arrGuiaSalidaDetalle as $row) {
    				$guia_detalle[] = array(
    			...
    			//Agregando flete
    			$arrFlete_ = array("ID_Guia_Cabecera" => $Last_ID_Guia_Cabecera);
    			$arrFlete = array_merge($arrFlete_, $arrFlete);
				...
				WHERE
					ID_Empresa				= " . $this->user->ID_Empresa . "
					AND ID_Tipo_Documento	= 7
					AND ID_Serie_Documento	= '" . $arrGuiaSalidaCabecera['ID_Serie_Documento'] . "'
				";
				$this->db->query($sql);
    			
    			//Solo para guia de salida
			    unset($arrGuiaSalidaCabecera['Txt_Direccion_Llegada']);
			    unset($arrGuiaSalidaCabecera['Txt_Referencia_Direccion_Llegada']);
			}
			
			if ($Nu_Tipo_Operacion === '3' || $Nu_Tipo_Operacion === '0') {//Factura Compra
			    $arrCompraCabecera = $arrGuiaSalidaCabecera;
			    $arrCompraDetalle = $arrGuiaSalidaDetalle;
    			...
    			if ( count($arrCorrelativoPendiente) > 0 ){
    				...
    				if($this->db->query("SELECT COUNT(*) existe FROM correlativo_tipo_asiento WHERE ID_Empresa = " . $this->user->ID_Empresa . " AND ID_Tipo_Asiento = 1 AND Fe_Year = '" . $Fe_Year . "' AND Fe_Month = '" . $Fe_Month . "' LIMIT 1")->row()->existe > 0){
				...
				WHERE
					...
					AND ID_Serie_Documento	= '" . $arrCompraCabecera['ID_Serie_Documento'] . "'
				";
				$this->db->query($sql);
			}
    ...
    public function actualizarGuiaSalida($where, $whereFacturaCompra, $arrGuiaSalidaCabecera, $arrGuiaSalidaDetalle, $EID_Tipo_Documento_Guia, $EID_Serie_Documento_Guia, $EID_Numero_Documento_Guia, $EID_Tipo_Documento_Factura, $EID_Serie_Documento_Factura, $EID_Numero_Documento_Factura, $arrFlete){
        $response = array('status' => 'error', 'style_modal' => 'modal-danger', 'message' => 'Error al modificar');
		if ( ($EID_Tipo_Documento_Guia != $arrGuiaSalidaCabecera['ID_Tipo_Documento_Guia'] || $EID_Serie_Documento_Guia != $arrGuiaSalidaCabecera['ID_Serie_Documento_Guia'] || $EID_Numero_Documento_Guia != $arrGuiaSalidaCabecera['ID_Numero_Documento_Guia']) && $arrGuiaSalidaCabecera['ID_Tipo_Operacion'] == '7' && $this->db->query("SELECT COUNT(*) existe FROM " . $this->table . " WHERE ID_Empresa = " . $this->user->ID_Empresa . " AND ID_Tipo_Documento = " . $arrGuiaSalidaCabecera['ID_Tipo_Documento_Guia'] . " AND ID_Serie_Documento = '" . $arrGuiaSalidaCabecera['ID_Serie_Documento_Guia'] . "' AND ID_Numero_Documento = '" . $arrGuiaSalidaCabecera['ID_Numero_Documento_Guia'] . "' LIMIT 1")->row()->existe > 0){
			$response = array('status' => 'warning', 'style_modal' => 'modal-warning', 'message' => 'El registro ya existe');
		} else if ( ($EID_Tipo_Documento_Factura != $arrGuiaSalidaCabecera['ID_Tipo_Documento_Factura'] || $EID_Serie_Documento_Factura != $arrGuiaSalidaCabecera['ID_Serie_Documento_Factura'] || $EID_Numero_Documento_Factura != $arrGuiaSalidaCabecera['ID_Numero_Documento_Factura']) && $arrGuiaSalidaCabecera['ID_Tipo_Operacion'] == '0' && $this->db->query("SELECT COUNT(*) existe FROM " . $this->table . " WHERE ID_Empresa = " . $this->user->ID_Empresa . " AND ID_Tipo_Documento = " . $arrGuiaSalidaCabecera['ID_Tipo_Documento_Factura'] . " AND ID_Serie_Documento = '" . $arrGuiaSalidaCabecera['ID_Serie_Documento_Factura'] . "' AND ID_Numero_Documento = '" . $arrGuiaSalidaCabecera['ID_Numero_Documento_Factura'] . "' LIMIT 1")->row()->existe > 0){
			$response = array('status' => 'warning', 'style_modal' => 'modal-warning', 'message' => 'El registro ya existe');
		} else {
			$this->db->trans_begin();
			...
			if ($Nu_Tipo_Operacion === '7' || $Nu_Tipo_Operacion === '0') {//Guia de Remision ó Factura + Guia
		        ...
		        $this->db->delete('flete', $where);
	        
	        	$arrGuiaSalidaCabecera['ID_Guia_Cabecera'] = $ID_Guia_Cabecera;
			    $arrGuiaSalidaCabecera['ID_Tipo_Asiento'] = $arrGuiaSalidaCabecera['ID_Tipo_Asiento_Guia'];
			    $arrGuiaSalidaCabecera['ID_Tipo_Documento'] = $arrGuiaSalidaCabecera['ID_Tipo_Documento_Guia'];
			    $arrGuiaSalidaCabecera['ID_Serie_Documento'] = $arrGuiaSalidaCabecera['ID_Serie_Documento_Guia'];
			    $arrGuiaSalidaCabecera['ID_Numero_Documento'] = $arrGuiaSalidaCabecera['ID_Numero_Documento_Guia'];
			...
			if ($Nu_Tipo_Operacion === '0') {//Factura Compra ó Factura + Guia
				...
				$Nu_Numero_Documento = $this->db->query("SELECT Nu_Numero_Documento FROM serie_documento WHERE ID_Empresa = " . $this->user->ID_Empresa . " AND ID_Tipo_Documento = 3 AND ID_Serie_Documento = '" . $arrCompraCabecera['ID_Serie_Documento'] . "' LIMIT 1")->row()->Nu_Numero_Documento;
				settype($Nu_Numero_Documento, "int");
				if ($Nu_Numero_Documento === 1) {//Solo entra una vez
					$sql = "
					UPDATE
						serie_documento
					SET
