<?php
class MedioPagoModel extends CI_Model{
    ...
    var $column_order = array('No_Empresa', 'No_Medio_Pago', 'Txt_Medio_Pago', 'No_Codigo_Sunat_FE', 'No_Codigo_Sunat_PLE');
    var $column_search = array('No_Empresa', 'No_Medio_Pago', 'Txt_Medio_Pago');
    var $order = array('No_Empresa' => 'asc', 'No_Medio_Pago' => 'asc', 'Txt_Medio_Pago' => 'asc');
	...
	public function _get_datatables_query(){
        ...
        if( $this->input->post('Filtros_MedioPago') == 'MedioPago' )
            $this->db->like('No_Medio_Pago', $this->input->post('Global_Filter'));
    
        $this->db->select('EMP.ID_Empresa, EMP.No_Empresa, ID_Medio_Pago, No_Medio_Pago, Txt_Medio_Pago, No_Codigo_Sunat_FE, No_Codigo_Sunat_PLE, Nu_Tipo, Nu_Tipo_Caja, Nu_Orden, TDESTADO.No_Class AS No_Class_Estado, TDESTADO.No_Descripcion AS No_Descripcion_Estado')
        ->from($this->table)
    ...
    public function agregarMedioPago($data){
		if($this->db->query("SELECT COUNT(*) AS existe FROM " . $this->table . " WHERE ID_Empresa = ".$data['ID_Empresa']." AND No_Medio_Pago='" . $data['No_Medio_Pago'] . "' LIMIT 1")->row()->existe > 0){
    ...
    public function actualizarMedioPago($where, $data, $ENo_Medio_Pago){
		if( $ENo_Medio_Pago != $data['No_Medio_Pago'] && $this->db->query("SELECT COUNT(*) AS existe FROM " . $this->table . " WHERE ID_Empresa = ".$data['ID_Empresa']." AND No_Medio_Pago='" . $data['No_Medio_Pago'] . "' LIMIT 1")->row()->existe > 0 ){
			return array('status' => 'warning', 'style_modal' => 'modal-warning', 'message' => 'El registro ya existe');
		}else{
