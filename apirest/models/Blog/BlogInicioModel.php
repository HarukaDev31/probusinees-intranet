<?php
class BlogInicioModel extends CI_Model{
    ...
    public function agregarInicio($data){
		...
		if($this->db->query("SELECT COUNT(*) AS existe FROM " . $this->table . " WHERE ID_Empresa=" . $data['ID_Empresa'] . " AND Nu_Tipo_Inicio=" . $data['Nu_Tipo_Inicio'] . " AND No_Slider='" . $data['No_Slider'] . "' LIMIT 1")->row()->existe > 0){
    ...
    public function actualizarInicio($where, $data, $ENo_Slider){
		...
		if( $ENo_Slider != $data['No_Slider'] && $this->db->query("SELECT COUNT(*) AS existe FROM " . $this->table . " WHERE ID_Empresa=" . $data['ID_Empresa'] . " AND Nu_Tipo_Inicio=" . $data['Nu_Tipo_Inicio'] . " AND No_Slider='" . $data['No_Slider'] . "' LIMIT 1")->row()->existe > 0 ){
			$response = array('status' => 'warning', 'style_modal' => 'modal-warning', 'message' => 'El registro ya existe');
		}else{
