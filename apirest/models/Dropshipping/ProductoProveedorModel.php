...
class ProductoProveedorModel extends CI_Model{
    ...
    var $order = array('PRO.Fe_Registro' => 'desc', 'Nu_Activar_Item_Lae_Shop' => 'desc');
        ...
        foreach ($this->_batchImport as $row) {
        	...
        	if ($this->db->query("SELECT COUNT(*) AS existe FROM " . $this->table . " WHERE ID_Empresa=" . $ID_Empresa . " AND Nu_Codigo_Barra='" . $row['Nu_Codigo_Barra'] . "' LIMIT 1")->row()->existe == 0){
            	$_arrProducto = array(
					...
					'ID_Ubicacion_Inventario' => $row['ID_Ubicacion_Inventario'],
	...
	public function _get_datatables_query(){
		...
		$this->db->select('CONFI.No_Tienda_Lae_Shop, PRO.ID_Empresa, PRO.ID_Producto, No_Producto, 0 AS Qt_Producto, F.No_Familia, PRO.Ss_Precio_Ecommerce_Online_Regular, PRO.Ss_Precio_Vendedor_Dropshipping, No_Imagen_Item, PRO.Nu_Version_Imagen')
		->from($this->table . ' AS PRO')
		->join('empresa AS EMP', 'EMP.ID_Empresa = PRO.ID_Empresa', 'join')
		->join('configuracion AS CONFI', 'CONFI.ID_Empresa = EMP.ID_Empresa', 'join')
		->join($this->table_familia . ' AS F', 'F.ID_Familia = PRO.ID_Familia', 'left')
		->where('EMP.Nu_Activar_Dropshipping', 1)
...
stock_producto
WHERE
ID_Producto = " . $arrParamsProductoProveedorDropshipping['ID_Producto'] . "
AND ID_Almacen = (SELECT
ID_Almacen
FROM
almacen
WHERE
Nu_Activar_Dropshipping=1
AND ID_Organizacion IN(SELECT ID_Organizacion FROM organizacion WHERE ID_Empresa = " . $arrParamsProductoProveedorDropshipping['ID_Empresa'] . ")
LIMIT 1)";
