<?php
class PedidoProveedoDropshippingModel extends CI_Model{
	public function __construct(){
		parent::__construct();
	}
	
    public function getReporte($arrParams){
		$where_id_empresa = " AND PD.ID_Empresa_Marketplace_Seller = " . $this->empresa->ID_Empresa;
		if($this->user->ID_Usuario == 1 && $arrParams['ID_Filtro_Empresa']==0){
			$where_id_empresa = "";
		} else if($this->user->ID_Usuario == 1 && $arrParams['ID_Filtro_Empresa']!=0){
			$where_id_empresa = " AND PD.ID_Empresa_Marketplace_Seller = " . $ID_Filtro_Empresa=$arrParams['ID_Filtro_Empresa'];
		}
		...
		//$where_pais_estado = 'AND PC.Nu_Estado IN(2,3,4,5)';
		//if($this->user->ID_Pais == 2) {
			$where_pais_estado = 'AND PC.Nu_Estado NOT IN(1,2)';
		//}
...
PC.ID_Pedido_Cabecera = " . $ID;
    ...
    public function getPrecioPedidoDetalleEcxlae($ID_Pedido_Cabecera, $ID_Pais){
		if($ID_Pais == 1) {
        	$query ="SELECT SUM(3 * Qt_Producto) AS Total_Precio_Ecxlae FROM pedido_detalle WHERE ID_Pedido_Cabecera = " . $ID_Pedido_Cabecera;
		} else if ($ID_Pais == 2) {
			$query ="SELECT SUM(20 * Qt_Producto) AS Total_Precio_Ecxlae FROM pedido_detalle WHERE ID_Pedido_Cabecera = " . $ID_Pedido_Cabecera;
		}
        return $this->db->query($query)->row()->Total_Precio_Ecxlae;
    }

    public function getPrecioPedidoDetalleProveedor($ID_Pedido_Cabecera){
        $query ="SELECT SUM(Ss_Precio_Empresa_Proveedor * Qt_Producto) AS Total_Precio_Proveedor FROM pedido_detalle WHERE ID_Pedido_Cabecera = " . $ID_Pedido_Cabecera;
        return $this->db->query($query)->row()->Total_Precio_Proveedor;
    }

	public function cambiarEstadoImpresionGuiaMexico($ID, $imprimir_guia_mexico, $Nu_Estado){
		if($imprimir_guia_mexico==0 && $Nu_Estado==8){//8=Guia Generada
			$where = array('ID_Pedido_Cabecera' => $ID);
			$data = array( 'Nu_Estado' => 3);//3=Preparando
			if ($this->db->update('pedido_cabecera', $data, $where) > 0)
				return array('status' => 'success', 'style_modal' => 'modal-success', 'message' => 'Estado cambiado satisfactoriamente');
			return array('status' => 'error', 'style_modal' => 'modal-danger', 'message' => 'Error al cambiar estado');
		} else {
			return array('status' => 'success', 'style_modal' => 'modal-success', 'message' => 'No actualizamos estado');
		}
	}
}
