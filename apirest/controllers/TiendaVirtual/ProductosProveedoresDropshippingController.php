...
class ProductosProveedoresDropshippingController extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('TiendaVirtual/ProductoProveedorModel');
		    ...
		    if (copy($_FILES['excel-archivo_producto']['tmp_name'], $destino)) {
		        if (file_exists($destino)) {
                	...
                	if ( count($arrProducto) > 0 ) {
						...
						redirect('TiendaVirtual/ProductosDropshippingController/listar/' . $sStatus . '/' . 0 . '/Error sin datos');
                		exit();
                	}
					...
					if ($arrResponseProducto['status'] == 'success') {
                		$sStatus = 'success';
						redirect('TiendaVirtual/ProductosDropshippingController/listar/' . $sStatus . '/' . $iCantidadNoProcesados . '/' . $arrResponseProducto['message']);
                	} else {
                		$sStatus = 'error-bd';
						redirect('TiendaVirtual/ProductosDropshippingController/listar/' . $sStatus . '/' . 0 . '/' . $arrResponseProducto['message']);
                	}
		        } else {
        	        $sStatus = 'error-archivo_no_existe';
					redirect('TiendaVirtual/ProductosDropshippingController/listar/' . $sStatus . '/' . 0 . '/No existe archivo');
		        }
		    } else {
		        $sStatus = 'error-copiar_archivo';
				redirect('TiendaVirtual/ProductosDropshippingController/listar/' . $sStatus . '/' . 0 . '/Error al copiar archivo');
		    }
		}
	}
	...
	public function listar($sStatus='', $iCantidadNoProcesados='', $sMessageErrorBD=''){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$arrUrlDropshipping = array();
			$this->load->view('header', array("arrUrlDropshipping" => $arrUrlDropshipping));
			$this->load->view('TiendaVirtual/ProductoProveedorView', array('sStatus' => $sStatus, 'iCantidadNoProcesados' => $iCantidadNoProcesados, 'sMessageErrorBD' => $sMessageErrorBD));
	...
	public function ajax_list(){
        ...
        foreach ($arrData as $row) {
			...
			$rows[] = $fStockActual->Qt_Producto;
			*/
        	settype($row->Qt_Producto, "double");
            $rows[] = numberFormat($row->Qt_Producto, 0, '.', '');

			$rows[] = numberFormat($row->Ss_Precio_Proveedor_Dropshipping, 2, '.', ',');
			$rows[] = numberFormat($row->Ss_Precio_Vendedor_Dropshipping, 2, '.', ',');
