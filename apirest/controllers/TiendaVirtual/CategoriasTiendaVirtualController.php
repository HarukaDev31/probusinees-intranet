...
class CategoriasTiendaVirtualController extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('TiendaVirtual/CategoriaModel');
	...
	public function listar(){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('TiendaVirtual/CategoriaView');
			$this->load->view('footer', array("js_categoria_tienda_virtual" => true));
		}
	}
        ...
        foreach ($arrData as $row) {
			...
			if(!empty($sPathImgProducto) && !empty($row->No_Imagen_Url_Categoria)){
				if ( file_exists($sPathImgProducto) ) {
					...
					$image = '<img class="img-fluid" data-url_img="' . $row->No_Imagen_Url_Categoria . '" src="' . $base64 . '" title="' . $row->No_Familia . '" alt="' . $row->No_Familia . '" style="cursor:pointer; max-height:40px;" />';
				}
			}
