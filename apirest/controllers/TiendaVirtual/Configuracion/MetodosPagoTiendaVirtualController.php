...
class MetodosPagoTiendaVirtualController extends CI_Controller {
	
	function __construct(){
    	parent::__construct();	
		$this->load->library('session');
		$this->load->database('LAE_SYSTEMS');
		$this->load->model('TiendaVirtual/Configuracion/MedioPagoModel');
	...
	public function listar(){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$iEstadoPasarelaPagoMercadoPago = $this->MedioPagoModel->buscarPasarelaPagoActivadas(2);

			$this->load->view('header', array("arrUrlTiendaVirtual" => $arrUrlTiendaVirtual));
			$this->load->view('TiendaVirtual/Configuracion/MedioPagoView', array(
				"iEstadoPasarelaPagoMercadoPago" => $iEstadoPasarelaPagoMercadoPago
			));
			$this->load->view('footer', array("js_metodo_pago_tienda_virtual" => true));
		}
	}
	...
	public function crudMedioPago(){
		...
		($this->input->post('EID_Medio_Pago') != '') ?
			$this->MedioPagoModel->actualizarMedioPago(array('ID_Medio_Pago' => $this->input->post('EID_Medio_Pago')), $data, $this->input->post('ENo_Medio_Pago_Tienda_Virtual'))
		:
	...
	public function ajax_list_cuentas_bancarias(){
		$arrData = $this->MedioPagoModel->get_datatables_cuentas_bancarias();
        $data = array();
        $no = $this->input->post('start');
        foreach ($arrData as $row) {
            $no++;
        ...
        $output = array(
	        'draw' => $this->input->post('draw'),
	        'recordsTotal' => $this->MedioPagoModel->count_all_cuentas_bancarias(),
	        'recordsFiltered' => $this->MedioPagoModel->count_filtered_cuentas_bancarias(),
	        'data' => $data,
        );
        echo json_encode($output);
    }
	...
	public function ajax_edit_cuentas_bancarias($ID){
        echo json_encode($this->MedioPagoModel->get_by_id_cuentas_bancarias($this->security->xss_clean($ID)));
    }
	...
	public function crudMedioPago_cuentas_bancarias(){
		...
		($this->input->post('EID_Cuenta_Bancaria') != '') ?
			$this->MedioPagoModel->actualizarMedioPago_cuentas_bancarias(array('ID_Cuenta_Bancaria' => $this->input->post('EID_Cuenta_Bancaria')), $data)
		:
			$this->MedioPagoModel->agregarMedioPago_cuentas_bancarias($data)
		);
	}
    
	public function eliminarMedioPago_cuentas_bancarias($ID){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
		echo json_encode($this->MedioPagoModel->eliminarMedioPago_cuentas_bancarias($this->security->xss_clean($ID)));
	}
	//FIN - NRO. CUENTAS BANCARIAS
    
	public function activarMercadoPago(){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
		echo json_encode($this->MedioPagoModel->activarMercadoPago());
	}
}
