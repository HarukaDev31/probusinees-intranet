...
class MarcasTiendaVirtualController extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('TiendaVirtual/MarcaModel');
	...
	public function listar(){
		...
		if(isset($this->session->userdata['usuario'])) {
		  $arrUrlTiendaVirtual = $this->ConfiguracionModel->obtenerUrlTiendaVirtual();
			...
			$this->load->view('TiendaVirtual/MarcaView');
			$this->load->view('footer', array("js_marca_tienda_virtual" => true));
		}
	}
    	...
    	if (!empty($_FILES)){
			...
			if ( !file_exists($path . '/' . cambiarCaracteresEspecialesImagen($_FILES['file']['name'])) ){
				...
				if (!$this->upload->do_upload('file')){
					...
					$this->MarcaModel->actualizarVersionImagen($where, $data);
    ...
    public function removeFileImage(){
		...
		if ( strpos($nameFileImage,"/") > 0 ) {
			if ( file_exists($nameFileImage) ){
				...
				$where = array('ID_Marca' => $this->input->post('iIdProducto') );
				echo json_encode($this->MarcaModel->actualizarVersionImagen($where, $data));
			} else {
