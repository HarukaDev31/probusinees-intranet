...
class CallCenterController extends CI_Controller {
	...
	public function listar(){
		...
		if(isset($this->session->userdata['usuario'])) {
		  $arrUrlTiendaVirtual = $this->ConfiguracionModel->obtenerUrlTiendaVirtual();
			...
			$this->load->view('CallCenterView');
			$this->load->view('footer', array("js_callcenter" => true));
		}
	}	
	
	public function verPedido($iIdPedido){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
		echo json_encode($this->CallCenterModel->verPedido($this->security->xss_clean($iIdPedido)));
  }
  
  private function getReporte($arrParams){
    $arrResponseModal = $this->CallCenterModel->getReporte($arrParams);
...
*/
  ...
  private function getReporteExcel($arrParams){
    $arrResponseModal = $this->CallCenterModel->getReporteExcel($arrParams);
    if ( $arrResponseModal['sStatus']=='success' ) {
      $data = array();
	...
	public function eliminarPedido($ID){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
    echo json_encode($this->CallCenterModel->eliminarPedido($this->security->xss_clean($ID)));
	}
  
	public function generarVenta(){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
    echo json_encode($this->CallCenterModel->generarVenta($this->input->post()));
	}
  ...
  public function sendReportePDF($Fe_Inicio, $Fe_Fin, $ID_Tipo_Documento, $ID_Pedido_Cabecera, $Nu_Estado_Pedido, $iIdCliente, $sNombreCliente, $ID_Filtro_Empresa, $Nu_Estado_Pedido_Empresa, $sNombreCiudad){
    ...
    if($arrCabeceraPedido['EID_Pedido_Cabecera'] != ''){//editar
      ...
      $response = $this->CallCenterModel->actualizarPedido(array('ID_Pedido_Cabecera' => $arrCabeceraPedido['EID_Pedido_Cabecera']), $data);
      echo json_encode($response);
      exit();
    } else {
      $response = $this->CallCenterModel->agregarPedido($arrCabeceraPedido, $arrPedidoDetalle);
      echo json_encode($response);
      exit();
    }
	}
  ...
  public function ajax_edit($ID){
    echo json_encode($this->CallCenterModel->get_by_id($this->security->xss_clean($ID)));
  }
	
	public function cambiarEstadoPedidoEmpresa($ID, $Nu_Estado){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
    	echo json_encode($this->CallCenterModel->cambiarEstadoPedidoEmpresa($this->security->xss_clean($ID), $this->security->xss_clean($Nu_Estado)));
	}
    ...
    $response = $this->CallCenterModel->actualizarPrecioDelivery(array('ID_Pedido_Cabecera' => $arrDataDelivery['id_pedido_cabecera']), $data);
    echo json_encode($response);
    exit();
	}
    ...
    $response = $this->CallCenterModel->actualizarNotaPedido(array('ID_Pedido_Cabecera' => $arrDataNotaPedido['id_pedido_cabecera']), $data);
    echo json_encode($response);
    exit();
	}
    ...
    $response = $this->CallCenterModel->asignarUsuarioDelivery(array('ID_Pedido_Cabecera' => $arrDataNotaPedido['id_pedido_cabecera']), $data);
    echo json_encode($response);
    exit();
	}
	...
	public function generarGuia99($ID){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
    echo json_encode($this->CallCenterModel->generarGuia99($this->security->xss_clean($ID)));
	}
  ...
  public function generarGuiaQuiken($ID){
    if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
    echo json_encode($this->CallCenterModel->generarGuiaQuiken($this->security->xss_clean($ID)));
  }
	
  public function generarGuiaEcxlae($ID){
    if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
    echo json_encode($this->CallCenterModel->generarGuiaEcxlae($this->security->xss_clean($ID)));
  }
}
