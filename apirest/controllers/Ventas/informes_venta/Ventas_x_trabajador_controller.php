...
class Ventas_x_trabajador_controller extends CI_Controller {
	
	function __construct(){
    	parent::__construct();	
		$this->load->library('session');
		$this->load->database('LAE_SYSTEMS');
		$this->load->model('Ventas/informes_venta/Ventas_x_trabajador_model');
	...
	public function listar(){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('Ventas/informes_venta/ventas_x_trabajador_view');
			$this->load->view('footer', array("js_ventas_x_trabajador" => true));
		}
    }
	
    private function getReporte($arrParams){
        $arrResponseModal = $this->Ventas_x_trabajador_model->getReporte($arrParams);
        if ( $arrResponseModal['sStatus']=='success' ) {
            ...
            foreach ($arrResponseModal['arrData'] as $row) {
                ...
                if(empty($row->ID_Entidad)) {
                    $arrParams = array( 'ID_Entidad' => $row->ID_Mesero );
                    $row_vendedor_fv = $this->HelperModel->getEntidad($arrParams);
                    if($row_vendedor_fv['sStatus']=='success'){
                        $row->ID_Entidad = $row_vendedor_fv['arrData'][0]->ID_Entidad;
                        $row->Nu_Documento_Identidad = $row_vendedor_fv['arrData'][0]->Nu_Documento_Identidad;
                        $row->No_Entidad = $row_vendedor_fv['arrData'][0]->No_Entidad;
                    }
                }
                
                $rows['ID_Entidad'] = ( !empty($row->ID_Entidad) ? $row->ID_Entidad : 'Sin personal' );
                ...
                $rows['ID_Numero_Documento'] = (int)$row->ID_Numero_Documento;
                ...
                if ( $row->Nu_Valor_Sunat_Moneda != 1) {//Moneda extranjera
            ...
            $orderID_Entidad = array();
            foreach ($data as $key => $row) {
                $orderID_Almacen[$key] = $row->ID_Almacen;
                $orderID_Entidad[$key] = $row->ID_Entidad;
            }
            array_multisort($orderID_Almacen, SORT_ASC, $orderID_Entidad, SORT_ASC, $data);
	...
	public function sendReporte(){
        $arrParams = array(
            ...
            'Nu_Tipo_Venta' => $this->input->post('Nu_Tipo_Venta')
        );
        echo json_encode($this->getReporte($arrParams));
    }
    
	public function sendReportePDF($Fe_Inicio, $Fe_Fin, $ID_Tipo_Documento, $ID_Serie_Documento, $ID_Numero_Documento, $Nu_Estado_Documento, $iIdEmpleado, $sNombreEmpleado, $iIdItem, $sNombreItem, $iTipoReporte, $ID_Almacen, $Nu_Tipo_Impuesto, $Nu_Tipo_Venta){
        ...
        $Nu_Tipo_Venta = $this->security->xss_clean($Nu_Tipo_Venta);
        
		$fileNamePDF = "Reporte_Ventas_x_Trabajador_" . $Fe_Inicio . "_" . $Fe_Fin . ".pdf";
		...
		ob_start();
		$file = $this->load->view('Ventas/informes_venta/pdf/ventas_x_trabajador_view_pdf', array(
		...
		$pdf->SetTitle('Laesystems - Reporte Ventas por Trabajador');
	...
	public function sendReporteEXCEL($Fe_Inicio, $Fe_Fin, $ID_Tipo_Documento, $ID_Serie_Documento, $ID_Numero_Documento, $Nu_Estado_Documento, $iIdEmpleado, $sNombreEmpleado, $iIdItem, $sNombreItem, $iTipoReporte, $ID_Almacen, $Nu_Tipo_Impuesto, $Nu_Tipo_Venta){
		...
		$fileNameExcel = "Reporte_Ventas_x_Trabajador_" . $Fe_Inicio . "_" . $Fe_Fin . ".xls";
		
	    $objPHPExcel = new PHPExcel();
	    
	    $objPHPExcel->getActiveSheet()->setTitle('Ventas por Trabajador');
        ...
        ->setCellValue('E2', 'Informe de Ventas por Trabajador')
