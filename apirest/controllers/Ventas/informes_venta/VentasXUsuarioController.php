...
class VentasXUsuarioController extends CI_Controller {
	
	function __construct(){
    	parent::__construct();	
		$this->load->library('session');
		$this->load->database('LAE_SYSTEMS');
		$this->load->model('Ventas/informes_venta/VentasXUsuariosModel');
	...
	public function listar(){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('Ventas/informes_venta/VentasXUsuarioView');
			$this->load->view('footer', array("js_ventas_x_usuario" => true));
		}
    }
	
    private function getReporte($arrParams){
        $arrResponseModal = $this->VentasXUsuariosModel->getReporte($arrParams);
        if ( $arrResponseModal['sStatus']=='success' ) {
            ...
            foreach ($arrResponseModal['arrData'] as $row) {
                ...
                //get trabajador                
                $rows['ID_Usuario'] = $row->ID_Usuario;
                $rows['Nu_Documento_Identidad'] = '-';
                $rows['No_Entidad'] = $row->No_Entidad . ' - Usuario: ' . $row->No_Usuario;
            ...
            $orderID_Usuario = array();
            foreach ($data as $key => $row) {
                $orderID_Almacen[$key] = $row->ID_Almacen;
                $orderID_Usuario[$key] = $row->ID_Usuario;
            }
            array_multisort($orderID_Almacen, SORT_ASC, $orderID_Usuario, SORT_ASC, $data);
	...
	public function sendReporteEXCEL($Fe_Inicio, $Fe_Fin, $ID_Tipo_Documento, $ID_Serie_Documento, $ID_Numero_Documento, $Nu_Estado_Documento, $iIdEmpleado, $sNombreEmpleado, $iIdItem, $sNombreItem, $iTipoReporte, $ID_Almacen, $Nu_Tipo_Impuesto, $Nu_Tipo_Venta){
        ...
        if ( $arrData['sStatus'] == 'success' ) {
            ...
            foreach ($arrData['arrData'] as $row) {
                if ($ID_Usuario != $row->ID_Usuario || $ID_Almacen != $row->ID_Almacen) {
                    if ($counter != 0) {
                        $objPHPExcel->setActiveSheetIndex($hoja_activa)
