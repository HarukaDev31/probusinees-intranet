...
class VentasxMarcaController extends CI_Controller {
	
	function __construct(){
    	parent::__construct();	
		$this->load->library('session');
		$this->load->database('LAE_SYSTEMS');
		$this->load->model('Ventas/informes_venta/VentasxMarcaModel');
	...
	public function listar(){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('Ventas/informes_venta/VentasxMarcaView');
			$this->load->view('footer', array("js_ventas_x_marca" => true));
		}
	}	
	
    private function getReporte($arrParams){
        $arrResponseModal = $this->VentasxMarcaModel->getReporte($arrParams);
        if ( $arrResponseModal['sStatus']=='success' ) {
            ...
            foreach ($arrResponseModal['arrData'] as $row) {
                ...
                $rows['ID_Marca'] = $row->ID_Marca;
                $rows['No_Marca'] = (!empty($row->No_Marca) ? $row->No_Marca : 'Sin Marca');
	...
	public function sendReportePDF($Fe_Inicio, $Fe_Fin, $iIdMoneda, $iIdFamilia, $iIdItem, $sNombreItem, $iIdSubFamilia, $ID_Almacen, $Nu_Tipo_Impuesto){
		...
		ob_start();
		$file = $this->load->view('Ventas/informes_venta/pdf/VentasxMarcaViewPDF', array(
		...
		$pdf->SetTitle('Laesystems - Informes de Ventas x Marca');
	...
	public function sendReporteEXCEL($Fe_Inicio, $Fe_Fin, $iIdMoneda, $iIdFamilia, $iIdItem, $sNombreItem, $iIdSubFamilia, $ID_Almacen, $Nu_Tipo_Impuesto){
		...
		$fileNameExcel = "reporte_ventas_x_marca_" . $Fe_Inicio . "_" . $Fe_Fin . ".xls";
		
	    $objPHPExcel = new PHPExcel();
	    
	    $objPHPExcel->getActiveSheet()->setTitle('Informe de Ventas x Marca');
        ...
        ->setCellValue('D2', 'Informe de Ventas por Marca')
        ...
        if ( $arrData['sStatus'] == 'success' ) {
            ...
            foreach($arrData['arrData'] as $row) {
                if ($ID_Marca != $row->ID_Marca || $ID_Almacen != $row->ID_Almacen) {
                    if ($counter != 0) {
                        ...
                        ->setCellValue('J' . $fila, numberFormat($sum_cantidad, 2, '.', ','))
                    ...
                    ->setCellValue('B' . $fila, $row->No_Marca)
                    ;
                ...
                ->setCellValue('J' . $fila, numberFormat($row->Qt_Producto, 6, '.', ','))
                ->setCellValue('K' . $fila, numberFormat($row->Ss_Precio, 6, '.', ','))
                ->setCellValue('L' . $fila, numberFormat($row->Ss_Subtotal, 6, '.', ','))
                ->setCellValue('M' . $fila, numberFormat($row->Ss_Impuesto, 6, '.', ','))
                ->setCellValue('N' . $fila, numberFormat($row->Ss_Total, 6, '.', ','))
            ...
            ->setCellValue('J' . $fila, numberFormat($sum_general_cantidad, 2, '.', ','))
