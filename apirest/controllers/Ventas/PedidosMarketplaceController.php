...
class PedidosMarketplaceController extends CI_Controller {
	
	function __construct(){
    parent::__construct();	
		$this->load->library('session');
		$this->load->database('LAE_SYSTEMS');
		$this->load->model('Ventas/PedidosMarketplaceModel');
	...
	public function listar(){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('Ventas/PedidosMarketplaceView');
			$this->load->view('footer', array("js_pedidos_marketplace" => true));
		}
	}	
	
	public function verPedido($iIdPedido){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
		echo json_encode($this->PedidosMarketplaceModel->verPedido($this->security->xss_clean($iIdPedido)));
  }
  
  private function getReporte($arrParams){
    $arrResponseModal = $this->PedidosMarketplaceModel->getReporte($arrParams);
  ...
  public function sendReportePDF($Fe_Inicio, $Fe_Fin, $ID_Tipo_Documento, $ID_Pedido_Cabecera, $Nu_Estado_Pedido, $iIdCliente, $sNombreCliente){
    ...
    ob_start();
    $file = $this->load->view('Ventas/pdf/PedidosMarketplaceViewPDF', array(
    ...
    ->setCellValue('E2', 'Informe de Ventas de PedidosMarketplace Online')
