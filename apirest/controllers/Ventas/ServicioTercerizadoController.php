...
class ServicioTercerizadoController extends CI_Controller {
	
	function __construct(){
    	parent::__construct();	
		$this->load->library('session');
		$this->load->database('LAE_SYSTEMS');
		$this->load->model('Ventas/ServicioTercerizadoModel');
	...
	public function listar(){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('Ventas/ServicioTercerizadoView');
			$this->load->view('footer', array("js_servicio_tercerizado" => true));
		}
	}
	
	public function getDocumento(){
        echo json_encode($this->ServicioTercerizadoModel->getDocumento($this->input->post()));
    }
	
	public function modificarPedido(){
        echo json_encode($this->ServicioTercerizadoModel->modificarPedido($this->input->post()));
    }
	...
	public function cambiarEstadoLavado(){
		...
		if ( !isset($_POST['arrIdDocumentoCabecera']) ){
			...
			echo json_encode($this->ServicioTercerizadoModel->cambiarEstadoLavado($this->input->post()));
			exit();
        }
	}
	
	public function getDocumentoProcesadoLavado(){
        echo json_encode($this->ServicioTercerizadoModel->getDocumentoProcesadoLavado($this->input->post()));
    }
	
	public function verificarPedido(){
        if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
		if ( !isset($_POST['arrIdItem']) ){
			echo json_encode($response = array('sStatus' => 'danger', 'sMessage' => 'Debe seleccionar al menos 1 fila'));
			exit();
		} else {
			echo json_encode($this->ServicioTercerizadoModel->verificarPedido($this->input->post()));
			exit();
        }
	}
	
    private function getReporte($arrParams){
        $arrResponseModal = $this->ServicioTercerizadoModel->getReporte($arrParams);
        if ( $arrResponseModal['sStatus']=='success' ) {
            ...
            foreach ($arrResponseModal['arrData'] as $row) {
                ...
                $arrEstadoLavadoInterno = $this->HelperModel->obtenerEstadoLavadoInternoArray($row->Nu_Estado_Lavado);
                ...
                $rows['No_Entidad_Proveedor'] = !empty($row->No_Entidad_Proveedor) ? $row->No_Entidad_Proveedor : '';
                $data[] = (object)$rows;
            }
            return array(
                'sStatus' => 'success',
	...
	public function sendReportePDF($Fe_Inicio, $Fe_Fin, $iIdTipoDocumento, $iIdSerieDocumento, $iNumeroDocumento, $iEstado, $iIdCliente, $sNombreCliente, $iEstadoLavado, $iIdProveedor){
		...
		ob_start();
		$file = $this->load->view('Ventas/pdf/ServicioTercerizadoViewPDF', array(
	...
	public function sendReporteEXCEL($Fe_Inicio, $Fe_Fin, $iIdTipoDocumento, $iIdSerieDocumento, $iNumeroDocumento, $iEstado, $iIdCliente, $sNombreCliente, $iEstadoLavado){
