...
class PrelavadoController extends CI_Controller {
	
	function __construct(){
    	parent::__construct();	
		$this->load->library('session');
		$this->load->database('LAE_SYSTEMS');
		$this->load->model('Ventas/PrelavadoModel');
	...
	public function listar(){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('Ventas/PrelavadoView');
			$this->load->view('footer', array("js_prelavado" => true));
		}
	}
	...
	public function agregarPedido(){
        ...
        echo json_encode($this->PrelavadoModel->agregarPedido($this->input->post()));
	}

	public function actualizarPedido(){
        if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
        echo json_encode($this->PrelavadoModel->actualizarPedido($this->input->post()));
	}
	
    private function getReporte($arrParams){
        $arrResponseModal = $this->PrelavadoModel->getReporte($arrParams);
        if ( $arrResponseModal['sStatus']=='success' ) {
            ...
            foreach ($arrResponseModal['arrData'] as $row) {
                ...
                $rows['ID_Documento_Detalle'] = $row->ID_Documento_Detalle;
                $rows['ID_Producto'] = $row->ID_Producto;
                $rows['Qt_Producto'] = $row->Qt_Producto;
                $rows['No_Producto'] = $row->No_Producto . ( !empty( $row->Txt_Nota_Item ) ? ' ' . $row->Txt_Nota_Item : '' );
                ...
                $iEstadoFinal = (!empty($row->Nu_Estado_Lavado_Detalle) ? $row->Nu_Estado_Lavado_Detalle : $row->Nu_Estado_Lavado);
                $arrEstadoLavadoInterno = $this->HelperModel->obtenerEstadoLavadoInternoArray($iEstadoFinal);
                $rows['No_Estado_Lavado'] = $arrEstadoLavadoInterno['No_Estado'];
                $rows['No_Class_Estado_Lavado'] = $arrEstadoLavadoInterno['No_Class_Estado'];
                //$rows['No_Estado_Lavado'] = (!empty($row->Nu_Estado_Lavado_Detalle) ? $row->No_Estado_Lavado_Detalle : $row->No_Estado_Lavado);
                //$rows['No_Class_Estado_Lavado'] = (!empty($row->Nu_Estado_Lavado_Detalle) ? $row->No_Class_Estado_Lavado_Detalle : $row->No_Class_Estado_Lavado);
                
                //$rows['Nu_Estado_Lavado'] = (!empty($row->Nu_Estado_Lavado_Detalle) ? $row->Nu_Estado_Lavado_Detalle : $row->Nu_Estado_Lavado);
                $rows['Nu_Estado_Lavado'] = $iEstadoFinal;
                $rows['Nu_Estado_Lavado_Detalle'] = $row->Nu_Estado_Lavado_Detalle;
                
                $rows['Txt_Glosa'] = $row->Txt_Glosa;

                $arrEstadoLavadoItemInterno = $this->HelperModel->obtenerEstadoLavadoItemInternoArray($row->ID_Tipo_Pedido_Lavado);
                $rows['No_Tipo_Pedido_Lavado'] = $arrEstadoLavadoItemInterno['No_Estado'];
	...
	public function sendReportePDF($Fe_Inicio, $Fe_Fin, $iIdTipoDocumento, $iIdSerieDocumento, $iNumeroDocumento, $iEstado, $iEstadoLavado, $iIdFamilia, $iIdItem, $sNombreItem, $iIdCliente, $sNombreCliente){
		...
		ob_start();
		$file = $this->load->view('Ventas/pdf/PrelavadoViewPDF', array(
	...
	public function sendReporteEXCEL($Fe_Inicio, $Fe_Fin, $iIdTipoDocumento, $iIdSerieDocumento, $iNumeroDocumento, $iEstado, $iEstadoLavado, $iIdFamilia, $iIdItem, $sNombreItem, $iIdCliente, $sNombreCliente){
