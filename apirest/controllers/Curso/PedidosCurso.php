...
class PedidosCurso extends CI_Controller {
	...
	public function ajax_edit($ID){
        echo json_encode($this->PedidosCursoModel->get_by_id($this->security->xss_clean($ID)));
    }

	public function cambiarEstado($ID, $Nu_Estado, $id_correlativo){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
    	echo json_encode($this->PedidosCursoModel->cambiarEstado($this->security->xss_clean($ID), $this->security->xss_clean($Nu_Estado), $this->security->xss_clean($id_correlativo)));
	}
	...
	public function crudPedidoGrupal(){
		...
		echo json_encode($this->PedidosCursoModel->actualizarPedido(
				array(
					'ID_Pedido_Curso' => $this->input->post('EID_Pedido_Curso'),
	...
	public function generarPDFPedidoCliente($ID){
        $data = $this->PedidosCursoModel->get_by_id($this->security->xss_clean($ID));
		//array_debug($data);
		...
		if( !empty($data) ){
			...
			ob_start();
			$file = $this->load->view('Curso/pdf/PedidosCursoPDFView', array(
				'arrDataEmpresa' => $data,
				'arrData' => $data,
			...
			$pdf->SetTitle('ProBusiness_Cotizacion_' . $data[0]->Nu_Documento_Identidad . '_' . $data[0]->ID_Pedido_Curso);
			...
			$file_name = 'ProBusiness_Cotizacion_' . $data[0]->Nu_Documento_Identidad . '_' . $data[0]->ID_Pedido_Curso . ".pdf";
	...
	public function downloadImage($id){
		//echo "hola";
		$objPedido = $this->PedidosCursoModel->getDownloadImage($this->security->xss_clean($id));
	...
	public function asignarPedido($ID){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
    	echo json_encode($this->PedidosCursoModel->asignarPedido($this->security->xss_clean($ID)));
	}

	public function removerAsignarPedido($ID, $id_usuario){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
    	echo json_encode($this->PedidosCursoModel->removerAsignarPedido($this->security->xss_clean($ID), $this->security->xss_clean($id_usuario)));
	}
}
