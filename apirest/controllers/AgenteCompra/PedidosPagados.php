...
class PedidosPagados extends CI_Controller
{
    ...
    public function ajax_list()
    {
        ...
        foreach ($arrData as $row) {
			...
			onclick='getOrderProgress(" . $row->ID_Pedido_Cabecera . ",".$row->Nu_Tipo_Servicio.")'><i class='fas fa-eye fa-2x' aria-hidden='true'></i></button>";
    ...
    public function saveInspectionPhotos()
    {
        $data = $this->input->post();
        $files = $_FILES;
        $response = $this->PedidosPagadosModel->saveInspectionPhotos($data, $files);
        echo json_encode(array('status' => 'success', 'data' => $response));
    }
    ...
    public function deleteDocumentationFolders(){
        $data = $this->input->post();
        $id=$data['id'];
        $idOrder=$data['idOrder'];
