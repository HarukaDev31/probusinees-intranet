...
class MetodoEntregaGrupal extends CI_Controller {
	...
	public function listar(){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('ImportacionGrupal/MetodoEntregaGrupalView');
			$this->load->view('footer_v2', array("js_metodo_entrega_grupal" => true));
		}
	}
	
	public function ajax_list(){
		$arrData = $this->MetodoEntregaGrupalModel->get_datatables();
        $data = array();
	...
	public function ajax_edit($ID){
        echo json_encode($this->MetodoEntregaGrupalModel->get_by_id($this->security->xss_clean($ID)));
    }
	...
	public function crudMedioPago(){
		...
		echo json_encode($this->MetodoEntregaGrupalModel->actualizarMetodoEntrega(array('ID_Metodo_Entrega_Tienda_Virtual' => $this->input->post('EID_Metodo_Entrega_Tienda_Virtual')), $data, $this->input->post('ENo_Metodo_Entrega_Tienda_Virtual'), $this->input->post('ENu_Tipo_Metodo_Entrega_Tienda_Virtual'), $data_recojo_tienda));
	}

	//DISTRITO	
	public function ajax_list_distrito(){
		$arrData = $this->MetodoEntregaGrupalModel->get_datatables_distrito();
        $data = array();
        $action = 'delete';
        foreach ($arrData as $row) {
            $rows = array();
			...
			<ul class="dropdown-menu" style="width: 100%; position: sticky;">
				<li class="dropdown-item"><a alt="Mostrar item en tienda" title="Mostrar lugar en tienda" href="javascript:void(0)" onclick="cambiarEstadoTienda(\'' . $row->ID_Distrito . '\',1);">Visible</a></li>
				<li class="dropdown-item"><a alt="Ocultar item en tienda" title="Ocultar lugar en tienda" href="javascript:void(0)" onclick="cambiarEstadoTienda(\'' . $row->ID_Distrito . '\',0);">Oculto</a></li>
			</ul>
			</div>';
			...
			$rows[] = '<button class="btn btn-xs btn-link" alt="Modificar" title="Modificar" href="javascript:void(0)" onclick="verDistrito(\'' . $row->ID_Distrito . '\')"><i class="far fa-edit fa-2x" aria-hidden="true"></i></button>';
            $data[] = $rows;
        }
        $output = array(
	        'data' => $data,
	...
	public function ajax_edit_distrito($ID){
        echo json_encode($this->MetodoEntregaGrupalModel->get_by_id_distrito($this->security->xss_clean($ID)));
    }
	...
	public function crudDistrito(){
		...
		echo json_encode($this->MetodoEntregaGrupalModel->actualizarDistrito(array('ID_Distrito' => $this->input->post('EID_Distrito')), $data));
	}
	//FIN DISTRITO
	
	public function updPrecioEstandarDelivery(){
        echo json_encode($this->MetodoEntregaGrupalModel->updPrecioEstandarDelivery($this->input->post()));
    }
	...
	function crudPromoDelivery() {
		...
		echo json_encode($this->MetodoEntregaGrupalModel->updatePromoDelivery($data));
	}
	//FIN CRUD PROMO DELIVERY

	//AJAX GET PROMO DELIVERY
	public function ajax_promoDelivery(){
		echo json_encode($this->MetodoEntregaGrupalModel->get_promoDelivery());
    }
	//FIN AJAX GET PROMO DELIVERY
	
	public function cambiarEstadoTienda($ID, $Nu_Estado){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
    	echo json_encode($this->MetodoEntregaGrupalModel->cambiarEstadoTienda($this->security->xss_clean($ID), $this->security->xss_clean($Nu_Estado)));
	}
}
