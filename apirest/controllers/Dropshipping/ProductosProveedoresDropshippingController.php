...
class ProductosProveedoresDropshippingController extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('Dropshipping/ProductoProveedorModel');
	...
	public function listar($sStatus='', $iCantidadNoProcesados='', $sMessageErrorBD=''){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('Dropshipping/ProductoProveedorView', array('sStatus' => $sStatus, 'iCantidadNoProcesados' => $iCantidadNoProcesados, 'sMessageErrorBD' => $sMessageErrorBD));
			$this->load->view('footer', array("js_productos_proveedores_dropshipping" => true));
		}
	}
  ...
  public function DefaultImagen(){
	...
	public function crudProducto(){
		...
		//AQUI CAMBIAR POR PRECIO DE DROPSHIPPING
		if( $_POST['arrProducto']['Ss_Precio_Vendedor_Dropshipping'] < 0.10 ) {
		...
		*/
		...
		$data_producto = array(
			...
			'Ss_Precio_Ecommerce_Online_Regular' => $this->security->xss_clean($_POST['arrProducto']['Ss_Precio_Vendedor_Dropshipping']),
