...
class KardexTextilController extends CI_Controller {
	
	function __construct(){
    	parent::__construct();	
		$this->load->library('session');
		$this->load->database('LAE_SYSTEMS');
		$this->load->model('LibrosPLE/KardexTextilModel');
	...
	public function reporteKardex(){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('LibrosPLE/KardexTextilView');
			$this->load->view('footer', array("js_kardex_textil" => true));
		}
	}
	
	public function getTiposLibroSunat(){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
        echo json_encode($this->KardexTextilModel->getTiposLibroSunat($this->input->post('ID_Tipo_Asiento')));
	}
	
    private function getDataKardex($arrParams){
        $arrResponseModal = $this->KardexTextilModel->kardex($arrParams);
        $sVarianteMultiple = '';
    ...
    $iFiltroItemMovimiento
		...
		ob_start();
		$file = $this->load->view('LibrosPLE/pdf/KardexTextilPDF', array(
    ...
    $iFiltroItemMovimiento
		...
		$fileNameExcel = "KardexFisico_" . $dInicio . "_" . $dFin . ".xls";
        $arrParams = array(
            "ID_Tipo_Asiento" => $ID_Tipo_Asiento,
            "ID_Tipo_Asiento_Detalle" => $ID_Tipo_Asiento_Detalle,
            "ID_Almacen" => $ID_Almacen,
        ...
        if ( $arrData['sStatus'] == 'success' ) {
            ...
            for ($i=0; $i < count($arrData['arrData']); $i++) {
                ...
                if ($ID_Producto != $row->ID_Producto || $ID_Almacen != $row->ID_Almacen) {
                    if ($counter != 0) {
                        ...
                        ->setCellValue('K' . $fila, $Qt_Producto_Saldo_Movimiento);
                    ...
                    if ($ID_Almacen != $row->ID_Almacen) {
                        if ($counter_almacen != 0) {
                            ...
                            ->setCellValue('K' . $fila, $fSumSaldoInicialxProductoAlmacen)
                            ->setCellValue('L' . $fila, ($fSumSaldoInicialxProductoAlmacen + $sum_Almacen_Producto_Qt_Entrada - $sum_Almacen_Producto_Qt_Salida));
                            $objPHPExcel->getActiveSheet()->getStyle('H' . $fila . ':L' . $fila)->getFont()->setBold(true);
                            $objPHPExcel->getActiveSheet()->getStyle('H' . $fila . ':L' . $fila)->applyFromArray($style_all_border_right);
                            $fila++;
                        }
                    ...
                    $fila++;
                    $fSumSaldoInicialxProductoAlmacen += $Qt_Producto_Saldo_Movimiento;
                }
            ...
            for ($p = 0; $p < count($arrData['arrDataAlmacenSinMovimiento'][0]); $p++) {
                $row_producto = $arrData['arrDataAlmacenSinMovimiento'][0];
                $fila++;
                $objPHPExcel->setActiveSheetIndex($hoja_activa)
                ->setCellValue('A' . $fila, 'PERÍODO:')
                ->setCellValue('E' . $fila, $fMonth . ' ' . $fYear);
                ...
                $objPHPExcel->setActiveSheetIndex($hoja_activa)
                ->setCellValue('A' . $fila, 'ALMACÉN:')
                ->setCellValue('E' . $fila, $row_producto[$p]->No_Almacen);
