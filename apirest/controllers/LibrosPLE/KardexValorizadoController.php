...
class KardexValorizadoController extends CI_Controller {
	
	function __construct(){
    	parent::__construct();	
		$this->load->library('session');
		$this->load->database('LAE_SYSTEMS');
		$this->load->model('LibrosPLE/KardexValorizadoModel');
	...
	public function reporte(){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('LibrosPLE/KardexValorizadoView');
			$this->load->view('footer', array("js_kardex_valorizado" => true));
		}
	}
	
	public function getTiposLibroSunat(){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
        echo json_encode($this->KardexValorizadoModel->getTiposLibroSunat($this->input->post('ID_Tipo_Asiento')));
	}
	
    private function getDataKardex($arrParams){
        $arrResponseModal = $this->KardexValorizadoModel->kardex($arrParams);
        $sVarianteMultiple = '';
        if ( $arrResponseModal['sStatus']=='success' ) {
            ...
            foreach ($arrResponseModal['arrData'] as $row) {
                ...
                $rows['Ss_Costo_Prev_Rango_Fecha'] = $arrDataPrevSaldo['Ss_Costo_Prev_Rango_Fecha'];
                $rows['Ss_Importe_Inicial'] = $arrDataPrevSaldo['Ss_Importe_Prev_Rango_Fecha'];
	...
	public function kardex(){
     ...
     public function ReporteKardexValorizadoLista(){
        echo json_encode(array("sStatus"=>"success","arrData"=>$this->KardexValorizadoModel->getReporte_()));
  }

   public function CancelarReporte(){
        echo $this->KardexValorizadoModel->CancelarReporte($this->input->post("ID_Reporte"));
    }

    public function BajarReporte($ID_Reporte){
        $row = $this->KardexValorizadoModel->getReporteRow($ID_Reporte);
        if($row){
  ...
  public function CrearReporte(){
      echo $this->KardexValorizadoModel->CrearReporte($this->input->post());
  }

  public function ReporteKardexValorizadoBG(){
    $this->FileReporte = "";
    $this->FileName    = ""; 
    ...
    // $fYear = "2022";
    $row = $this->KardexValorizadoModel->getReporteBG();
    ...
    $this->KardexValorizadoModel->UpdateReporteBG($arr,$row->ID_Reporte);
	...
	public function kardexPDF($ID_Tipo_Asiento, $ID_Tipo_Asiento_Detalle, $ID_Almacen, $dInicio, $dFin, $ID_Producto, $Txt_Direccion_Almacen, $Nu_Codigo_Libro_Sunat, $No_Tipo_Asiento_Apertura, $ID_Tipo_Movimiento){
		...
		$fileNamePDF = "KardexValorizado_" . $dInicio . "_" . $dFin . ".pdf";
		...
		ob_start();
		$file = $this->load->view('LibrosPLE/pdf/KardexValorizadoPDF', array(
	...
	public function kardexEXCEL($ID_Tipo_Asiento, $ID_Tipo_Asiento_Detalle, $ID_Almacen, $dInicio, $dFin, $ID_Producto, $Txt_Direccion_Almacen, $Nu_Codigo_Libro_Sunat, $No_Tipo_Asiento_Apertura, $ID_Tipo_Movimiento){
		...
		$this->FileName = $fileNameExcel = "KardexValorizado_" . $dInicio . "_" . $dFin . ".xls";
        $arrParams = array(
            "ID_Tipo_Asiento" => $ID_Tipo_Asiento,
            "ID_Tipo_Asiento_Detalle" => $ID_Tipo_Asiento_Detalle,
            "ID_Almacen" => $ID_Almacen,
	    ...
	    $objPHPExcel->getActiveSheet()->setTitle('13 Reg. Inv. Perm. Valorizado');
        
	    $hoja_activa = 0;
	    
	    $objPHPExcel->getActiveSheet()->getStyle("A1")->getFont()->setBold(true);
        $objPHPExcel->setActiveSheetIndex($hoja_activa)
        ->setCellValue('A1', '13.1 REGISTRO DEL INVENTARIO PERMANENTE VALORIZADO - DETALLE DEL INVENTARIO VALORIZADO');
        ...
        if ( $arrData['sStatus'] == 'success' ) {
            ...
            for ($i=0; $i < count($arrData['arrData']); $i++) {
                ...
                if ($ID_Producto != $row->ID_Producto || $ID_Almacen != $row->ID_Almacen) {
                    if ($counter != 0) {
                        ...
                        ->setCellValue('K' . $fila, $sum_Producto_Ss_SubTotal_Entrada)
                        ->setCellValue('L' . $fila, $sum_Producto_Qt_Salida)
                        ->setCellValue('N' . $fila, $sum_Producto_Ss_SubTotal_Salida);
                        $objPHPExcel->getActiveSheet()->getStyle('H' . $fila . ':N' . $fila)->getFont()->setBold(true);
                        $objPHPExcel->getActiveSheet()->getStyle('H' . $fila . ':N' . $fila)->applyFromArray($style_all_border_right);
                        $fila++;
                    }
                    ...
                    if ($ID_Almacen != $row->ID_Almacen) {
                        if ($counter_almacen != 0) {
                            ...
                            ->setCellValue('K' . $fila, $sum_Almacen_Producto_Ss_SubTotal_Entrada)
                            ->setCellValue('L' . $fila, $sum_Almacen_Producto_Qt_Salida)
                            ->setCellValue('N' . $fila, $sum_Almacen_Producto_Ss_SubTotal_Salida);
                            $objPHPExcel->getActiveSheet()->getStyle('H' . $fila . ':N' . $fila)->getFont()->setBold(true);
                            $objPHPExcel->getActiveSheet()->getStyle('H' . $fila . ':N' . $fila)->applyFromArray($style_all_border_right);
                            $fila++;
                        }
                    ...
                    ->setCellValue('L' . $fila, 'SALIDAS')
                    ->setCellValue('O' . $fila, 'SALDO FINAL');
                    
                    $objPHPExcel->setActiveSheetIndex($hoja_activa)->mergeCells('I' . $fila . ':K' . $fila);//ENTRADAS
                    $objPHPExcel->setActiveSheetIndex($hoja_activa)->mergeCells('L' . $fila . ':N' . $fila);//SALIDAS
                    $objPHPExcel->setActiveSheetIndex($hoja_activa)->mergeCells('O' . $fila . ':Q' . $fila);//SALDO FINAL
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('N' . $fila)->applyFromArray($BStyle_right);
                    $objPHPExcel->getActiveSheet()->getStyle('O' . $fila)->applyFromArray($BStyle_right);
                    $objPHPExcel->getActiveSheet()->getStyle('P' . $fila)->applyFromArray($BStyle_right);
                    $objPHPExcel->getActiveSheet()->getStyle('Q' . $fila)->applyFromArray($BStyle_right);
                    $objPHPExcel->getActiveSheet()->getStyle('A' . $fila . ':E' . $fila)->applyFromArray($BStyle_bottom);
                    $objPHPExcel->getActiveSheet()->getStyle('I' . $fila . ':K' . $fila)->applyFromArray($BStyle_bottom);
                    $objPHPExcel->getActiveSheet()->getStyle('L' . $fila . ':N' . $fila)->applyFromArray($BStyle_bottom);
                    $objPHPExcel->getActiveSheet()->getStyle('O' . $fila . ':Q' . $fila)->applyFromArray($BStyle_bottom);
                    $objPHPExcel->getActiveSheet()->getStyle('A' . $fila . ':Q' . $fila)->applyFromArray($BStyle_top);
                    $objPHPExcel->getActiveSheet()->getStyle('A' . $fila . ':Q' . $fila)->getFont()->setBold(true);
                    ...
                    ->setCellValue('J' . $fila, 'C/UNITARIO')
                    ->setCellValue('K' . $fila, 'TOTAL')
                    ->setCellValue('L' . $fila, 'CANTIDAD')
                    ->setCellValue('M' . $fila, 'C/UNITARIO')
                    ->setCellValue('N' . $fila, 'TOTAL')
                    ->setCellValue('O' . $fila, 'CANTIDAD')
                    ->setCellValue('P' . $fila, 'C/UNITARIO')
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('P' . $fila)->applyFromArray($BStyle_right);
                    $objPHPExcel->getActiveSheet()->getStyle('Q' . $fila)->applyFromArray($BStyle_right);
                    $objPHPExcel->getActiveSheet()->getStyle('A' . $fila . ':Q' . $fila)->applyFromArray($BStyle_bottom);
                    $objPHPExcel->getActiveSheet()->getStyle('A' . $fila . ':Q' . $fila)->getFont()->setBold(true);

                    $fila++;
                    $objPHPExcel->setActiveSheetIndex($hoja_activa)
                    ->setCellValue('N' . $fila, 'SALDO INICIAL')
                    ->setCellValue('O' . $fila, numberFormat($row->Qt_Producto_Inicial, 2, '.', ''))
                    ->setCellValue('P' . $fila, numberFormat($row->Ss_Costo_Prev_Rango_Fecha, 2, '.', ''))
                    ->setCellValue('Q' . $fila, numberFormat($row->Ss_Importe_Inicial, 2, '.', ''));
                    $objPHPExcel->getActiveSheet()->getStyle('N' . $fila . ':Q' . $fila)->getFont()->setBold(true);
                    $objPHPExcel->getActiveSheet()->getStyle('N' . $fila)->applyFromArray($BStyle_left);
                    $objPHPExcel->getActiveSheet()->getStyle('O' . $fila)->applyFromArray($BStyle_right);
                    $objPHPExcel->getActiveSheet()->getStyle('P' . $fila)->applyFromArray($BStyle_right);
                    $objPHPExcel->getActiveSheet()->getStyle('Q' . $fila)->applyFromArray($BStyle_right);
                    $objPHPExcel->getActiveSheet()->getStyle('A' . $fila . ':Q' . $fila)->applyFromArray($BStyle_bottom);
                ...
                if ($row->Nu_Tipo_Movimiento == 0){
                    ...
                    ->setCellValue('L' . $fila, numberFormat($row->Qt_Producto, 2, '.', ''))
                    ->setCellValue('M' . $fila, numberFormat($row->Ss_Precio, 2, '.', ''))
                ...
                $objPHPExcel->setActiveSheetIndex($hoja_activa)
                ->setCellValue('O' . $fila, numberFormat($Qt_Producto_Saldo_Movimiento, 2, '.', ''))
                ->setCellValue('P' . $fila, numberFormat($row->Ss_Costo_Promedio, 6, '.', ''))
                ->setCellValue('Q' . $fila, numberFormat(($Qt_Producto_Saldo_Movimiento * $row->Ss_Costo_Promedio), 2, '.', ''));
            ...
            ->setCellValue('K' . $fila, $sum_General_Ss_Subtotal_Entrada)
            ->setCellValue('L' . $fila, $sum_General_Qt_Salida)
            ->setCellValue('N' . $fila, $sum_General_Ss_SubTotal_Salida);
            ...
            ++$fila;
            $objPHPExcel->setActiveSheetIndex($hoja_activa)
            ->setCellValue('H' . $fila, 'TOTAL IMPORTE (ENTRADA - SALIDA)')
            ->setCellValue('I' . $fila, $sum_General_Ss_Subtotal_Entrada - $sum_General_Ss_SubTotal_Salida);
            $objPHPExcel->setActiveSheetIndex($hoja_activa)->mergeCells('I'. $fila . ':J'. $fila);
