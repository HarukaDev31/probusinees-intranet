...
class ProductosProveedoresDropshippingController extends CI_Controller {
	...
	public function ajax_list(){
        ...
        foreach ($arrData as $row) {
			...
			if(!empty($sPathImgProducto) && !empty($row->No_Imagen_Item)){
				if ( file_exists($sPathImgProducto) ) {
					...
					if ($bStatusFileImage) {
						...
						$image = '<img class="img-fluid" data-url_img="' . $row->No_Imagen_Item . '" src="' . $base64 . '" title="' . limpiarCaracteresEspeciales($row->No_Producto) . '" alt="' . limpiarCaracteresEspeciales($row->No_Producto) . '" style="cursor:pointer; max-height:300px; border-radius: 12px;" />';
					}
				}
			}
			...
			$rows[] = '<button class="btn btn-xs btn-link" alt="Importar" title="Importar" href="javascript:void(0)" onclick="verProducto(\'' . $row->ID_Producto . '\', \'' . $row->No_Imagen_Item . '\', \'' . $row->Nu_Version_Imagen . '\')"><i class="fa fa-2x fa-plus-square" aria-hidden="true"></i></button>';			
			
			//$rows[] = $row->No_Empresa;
			//$rows[] = $row->No_Almacen;

			$rows[] = '<strong style="font-size: 1.6rem;">' . $row->No_Empresa . '</strong><br>' . 'Almacen: <strong style="font-size: 1.5rem;">' . $row->No_Almacen . '<br><br></strong>' . (!empty($image) ? $image : 'Sin imagen');
