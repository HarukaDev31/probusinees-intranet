...
class ProfesorController extends CI_Controller {
	
	function __construct(){
    	parent::__construct();	
		$this->load->library('session');
		$this->load->database('LAE_SYSTEMS');
		$this->load->model('Escuela/ProfesorModel');
	...
	public function listar(){
		...
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('Escuela/ProfesorView');
			$this->load->view('footer', array("js_escuela_profesor" => true));
		}
	}

	public function ajax_list(){
		$arrData = $this->ProfesorModel->get_datatables();
        $data = array();
        ...
        foreach ($arrData as $row) {
			...
			$rows[] = '<button class="btn btn-xs btn-link" alt="Modificar" title="Modificar" href="javascript:void(0)" onclick="verEmpleado(\'' . $row->ID_Entidad . '\')"><i class="fa fa-pencil" aria-hidden="true"> Modificar</i></button>';
			$rows[] = '<button class="btn btn-xs btn-link" alt="Eliminar" title="Eliminar" href="javascript:void(0)" onclick="eliminarEmpleado(\'' . $row->ID_Entidad . '\', \'' . $action . '\')"><i class="fa fa-trash-o" aria-hidden="true"> Eliminar</i></button>';
            $data[] = $rows;
        }
        $output = array(
	        'draw' => $this->input->post('draw'),
	        'recordsTotal' => $this->ProfesorModel->count_all(),
	        'recordsFiltered' => $this->ProfesorModel->count_filtered(),
	        'data' => $data,
        );
	...
	public function ajax_edit($ID){
        echo json_encode($this->ProfesorModel->get_by_id($this->security->xss_clean($ID)));
    }
	...
	public function crudEmpleado(){
		...
		($this->input->post('EID_Empresa') != '' && $this->input->post('EID_Entidad') != '') ?
			$this->ProfesorModel->actualizarEmpleado(array('ID_Empresa' => $this->input->post('EID_Empresa'), 'ID_Entidad' => $this->input->post('EID_Entidad')), $data, $this->input->post('ENu_Documento_Identidad'), $this->input->post('ENu_Pin_Caja'))
		:
			$this->ProfesorModel->agregarEmpleado($data)
		);
	}
    
	public function eliminarEmpleado($ID){
		if (!$this->input->is_ajax_request()) exit('No se puede eliminar y acceder');
		echo json_encode($this->ProfesorModel->eliminarEmpleado($this->security->xss_clean($ID)));
	}
}
