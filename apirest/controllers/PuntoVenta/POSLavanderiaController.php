...
class POSLavanderiaController extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('PuntoVenta/POSLavanderiaModel');
	...
	public function verPOS(){
		if(isset($this->session->userdata['usuario'])) {
			...
			$this->load->view('PuntoVenta/POSLavanderiaView',
			array(
				'bStatusOpcionProductoAdd' => $bStatusOpcionProductoAdd
			));
			$this->load->view('footer', array("js_pos_lavanderia" => true));
		}
	}
	...
	public function agregarVentaPos(){
		if(isset($this->session->userdata['usuario']) && isset($this->session->userdata['arrDataPersonal'])) {
			...
			echo json_encode($this->POSLavanderiaModel->agregarVentaPos($this->input->post()));
		} else {
