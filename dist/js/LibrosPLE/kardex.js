...
function ReloadReporte(){     
  ...
  url = base_url + 'LibrosPLE/KardexController/ReporteKardexLista';
...
$(function () {
  ...
  $("#btn-generar").click(function(){
    ...
    if ( $( '#cbo-TiposLibroSunatKardex' ).val() == 0 ) {
     ...
     url = base_url + 'LibrosPLE/KardexController/CrearReporte';
...
$(document).on("click",".btn-download",function(){
    window.open(base_url + 'LibrosPLE/KardexController/BajarReporte/'+$(this).data("valor"), "_blank");
  });

  $(document).on("click",".btn-cancelar",function(){
    url = base_url + 'LibrosPLE/KardexController/CancelarReporte';
      ...
      iFiltroItemMovimiento = $('[name="radio-filtro_item_movimiento"]:checked').attr('value');
        ...
        if (ID_Tipo_Asiento == 3) {
          ...
          $.post( url, arrPost, function( responseData ){
            if ( responseData.sStatus == 'success' ) {
                ...
                if ($ID_Producto != response[i].ID_Producto || $ID_Almacen != response[i].ID_Almacen) {
                  if ($counter != 0) {
                    tr_body +=
                      ...
                      + "<th class='text-right' colspan='2' style='background-color: #d0d0d08a;'></th>"
                      + "</tr>";
                  }
              ...
              "<tfoot>"
                ...
                +"<tr>"
                  ...
                  +"<th class='text-right' colspan='2'><span style='font-size: 15px;'>" + Math.round10(parseFloat($sum_General_Qt_Entrada) - parseFloat($sum_General_Qt_Salida), -3) + "</span></th>"
                +"</tr>"
              +"</tfoot>";
            } else {
              tr_body +=
            ...
            $ID_Almacen = 0;
            for (var i = 0; i < iTotalRegistros; i++) {
              if ($ID_Almacen != response[i][0].ID_Almacen) {
                tr_body += "<tr>"
                + "<th class='text-right'><span style='font-size: 15px;'>ALMACÉN</span></th>"
                + "<th class='text-left' colspan='15'><span style='font-size: 15px;'>" + response[i][0].No_Almacen + "</span></th>"
                + "</tr>";
                $ID_Almacen = response[i][0].ID_Almacen;
                
                for (var p = 0; p < response[i].length; p++) {
                  response_producto = response[i];
                  tr_body += "<tr>"
                  + "<th class='text-right'>UPC</th>"
                  + "<th class='text-left'>" + response_producto[p].Nu_Codigo_Barra + "</th>"
                  + "<th class='text-right'>SKU</th>"
                  + "<th class='text-left'>" + response_producto[p].No_Codigo_Interno + "</th>"
                  + "<th class='text-right'>Nombre</th>"
                  + "<th class='text-left' colspan='4'>" + response_producto[p].No_Producto + "</th>"
                  + "<th class='text-right' colspan='2'>SALDO INICIAL</th>"
        ...
        url = base_url + 'LibrosPLE/KardexController/kardexEXCEL/' + ID_Tipo_Asiento + '/' + ID_Tipo_Asiento_Detalle + '/' + ID_Almacen + '/' + dInicio + '/' + dFin + '/' + ID_Producto + '/' + encodeURIComponent(Txt_Direccion_Almacen) + '/' + Nu_Codigo_Libro_Sunat + '/' + No_Tipo_Asiento_Apertura + '/' + ID_Tipo_Movimiento + '/' + iFiltroBusquedaNombre + '/' + encodeURIComponent(sNombreItem) + '/' + ID_Familia + '/' + ID_Sub_Familia + '/' + ID_Marca + '/' + ID_Variante_Item + '/' + ID_Variante_Item_Detalle_1 + '/' + ID_Variante_Item2 + '/' + ID_Variante_Item_Detalle_2 + '/' + ID_Variante_Item3 + '/' + ID_Variante_Item_Detalle_3 + '/' + iFiltroItemMovimiento;
        window.open(url,'_blank');
        ...
        url = base_url + 'LibrosPLE/KardexController/kardexTXT/' + ID_Tipo_Asiento + '/' + ID_Tipo_Asiento_Detalle + '/' + ID_Almacen + '/' + dInicio + '/' + dFin + '/' + ID_Producto + '/' + encodeURIComponent(Txt_Direccion_Almacen) + '/' + Nu_Codigo_Libro_Sunat + '/' + No_Tipo_Asiento_Apertura + '/' + ID_Tipo_Movimiento + '/' + iFiltroBusquedaNombre + '/' + encodeURIComponent(sNombreItem) + '/' + ID_Familia + '/' + ID_Sub_Familia + '/' + ID_Marca + '/' + ID_Variante_Item + '/' + ID_Variante_Item_Detalle_1 + '/' + ID_Variante_Item2 + '/' + ID_Variante_Item_Detalle_2 + '/' + ID_Variante_Item3 + '/' + ID_Variante_Item_Detalle_3;
        window.open(url,'_blank');
