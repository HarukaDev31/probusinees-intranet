...
$(function () {
  ...
  url = base_url + 'Escuela/LocalController/ajax_list';
...
function agregarPos(){
  ...
  $( '[name="EID_Sede_Musica"]' ).val('');
  
	$( '#modal-Pos' ).on('shown.bs.modal', function() {
    $('[name="No_Sede_Musica"]').focus();
	})
...
function verPos(ID){
  ...
  url = base_url + 'Escuela/LocalController/ajax_edit/' + ID;
  $.ajax({
    url : url,
    type: "GET",
    dataType: "JSON",
    success: function(response){
      ...
      $( '[name="No_Sede_Musica"]' ).val( response.No_Sede_Musica );
...
function form_Pos(){
  if ( accion_serie=='add_serie' || accion_serie=='upd_serie' ) {
      ...
      url = base_url + 'Escuela/LocalController/crudPos';
    	$.ajax({
        type		  : 'POST',
        dataType	: 'JSON',
    		url		    : url,
...
function _eliminarPos($modal_delete, ID){
  ...
  url = base_url + 'Escuela/LocalController/eliminarPos/' + ID;
  $.ajax({
    url       : url,
    type      : "GET",
    dataType  : "JSON",
      ...
      $modal_delete.modal('hide');//modal del mensaje de eliminación
  	  ...
  	  $( '#modal-message' ).modal('show');
  	    ...
  	    reload_table_serie();
		    ...
		    $( '#txt-ID_Sede_Musica_Documento' ).val('');
  	    $( '.modal-message' ).addClass(response.style_modal);
  	    $( '.modal-title-message' ).text(response.message);
