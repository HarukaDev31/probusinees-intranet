...
$(function () {
  ...
  $( '#btn-cloud-api_empleado' ).click(function(){
    if ( $( '#txt-Nu_Documento_Identidad').val().length < 8 ) {
      ...
      var url_api = 'https://www.laesystems.com/librerias/reniec/partner/format/json/x-api-key/' + sTokenGlobal;
  ...
  url = base_url + 'Escuela/ProfesorController/ajax_list';
...
function verEmpleado(ID){
  ...
  url = base_url + 'Escuela/ProfesorController/ajax_edit/' + ID;
  $.ajax({
    url : url,
    type: "GET",
    dataType: "JSON",
...
function crudEmpleado(){
  ...
  url = base_url + 'Escuela/ProfesorController/crudEmpleado';
  $.ajax({
    type		  : 'POST',
    dataType	: 'JSON',
    url		    : url,
...
function _eliminarEmpleado($modal_delete, ID){
  ...
  url = base_url + 'Escuela/ProfesorController/eliminarEmpleado/' + ID;
  $.ajax({
    url       : url,
    type      : "GET",
    dataType  : "JSON",
