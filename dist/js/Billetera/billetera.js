...
$(function () {
  ...
  url = base_url + 'Billetera/BilleteraVirtualController/ajax_list';
  ...
  url = base_url + 'Billetera/BilleteraVirtualController/ajax_list_desembolso_pendiente';
  ...
  url = base_url + 'Billetera/BilleteraVirtualController/ajax_list_desembolso_pago';
  ...
  $('#cbo-banco').change(function () {
    $('.div-banco_datos').show();
    $('#label-nro_cuenta').html('Número de Cuenta <span class="label-advertencia">*</span>');
    $('#label-titular').html('Titular <span class="label-advertencia">*</span>');
    if($(this).val()==8 || $(this).val()==26){//ID=8 OTROS
      $('.div-banco_datos').hide();
      $('#label-nro_cuenta').html('Nombre <span class="label-advertencia">*</span>');
      $('#label-titular').html('Correo <span class="label-advertencia">*</span>');
      $('#label-titular').attr("placeholder", "Ejemplo: Binance, Paypal, etc");
    }
    
    $('.div-banco_mexico').show();
    //$('.div-cci').show();
    if($('#hidden-ID_Pais_Usuario').val()=='1'){//1=Perú
      ...
      if($(this).val()==8){//ID=8 OTROS
        $('.div-banco_datos').hide();
      }
    } else {
      $('.div-banco_mexico').hide();
      //$('.div-cci').hide();
      $('#cbo-tipo_cuenta').html('<option value="1">Cuenta de Ahorro</option>');
      $('#label-titular').html('Número de Beneficiario <span class="label-advertencia">*</span>');
      if($(this).val()==26){//ID=8 OTROS
        $('#label-titular').html('Correo <span class="label-advertencia">*</span>');
        $('#label-titular').attr("placeholder", "Ejemplo: Binance, Paypal, etc");
      }
    }
...
function agregarCuentaBancaria() {
  ...
  //listar banco
  $('#cbo-banco').html('<option value="" selected="selected">- Cargando -</option>');
  ...
  $.post(url, {}, function (response) {
    if (response.status == 'success') {
      ...
      for (var x = 0; x < l; x++) {
        $('#cbo-banco').append('<option value="' + response.result[x].ID_Banco + '">' + response.result[x].No_Banco_Siglas + '</option>');
      }
    } else {
  ...
  //MONEDA
  $('#cbo-moneda').html('<option value="" selected="selected">- Cargando -</option>');
  url = base_url + 'HelperController/getMonedas';
  $.post(url, function (response) {
    if ( response.length == 1 ) {
      $('#cbo-moneda').html('<option value="' + response[0]['ID_Moneda'] + '" data-no_signo="' + response[0]['No_Signo'] + '">' + response[0]['No_Moneda'] + '</option>');
    } else {
...
function form_CuentaBancaria() {
  ...
  url = base_url + 'Billetera/BilleteraVirtualController/crudCuentaBancaria';
  $.ajax({
    type: 'POST',
    dataType: 'JSON',
    url: url,
    ...
    success: function (response) {
      ...
      if (response.status == 'success') {
        $('#modal-cuenta_bancaria').modal('hide');
...
function eliminarCuentaBancaria(ID){
  ...
  $( '#btn-save-delete' ).off('click').click(function () {
    _eliminarCuentaBancaria($modal_delete, ID);
  });
}

function _eliminarCuentaBancaria($modal_delete, ID){
  $( '#modal-loader' ).modal('show');
  
  url = base_url + 'Billetera/BilleteraVirtualController/eliminarCuentaBancaria/' + ID;
  $.ajax({
    url       : url,
    type      : "GET",
    dataType  : "JSON",
