...
$(function () {
	...
	$( '#cbo-contador_item' ).change(function(){
	  if ( $(this).val() == 0 ){//Servicio
		  $('.modal-formulario_contador_tiempo_item').modal('hide');
	  } else {
		  $('.modal-formulario_contador_tiempo_item').modal('show');
      
    	$( '.modal-formulario_contador_tiempo_item' ).on('shown.bs.modal', function() {
    		$( '#txt-modal-Nu_Tiempo_Minutos_Contador_Item_Tienda' ).focus();
    	})
    }
	})

	// Mostrar tipo de producto para inventarios
	$( '#cbo-formulario_ver_item' ).change(function(){
	  if ( $(this).val() == 0 ){//Servicio
		  $('.modal-fomulario_item_tienda').modal('hide');
	  } else {
		  $('.modal-fomulario_item_tienda').modal('show');
    }
	})
  ...
  $('#btn-agregar_integraciones').click(function () {
    if ($(this).data('agregar_integraciones') == 1) {
      ...
      $('#btn-agregar_integraciones').text('ocultar');
    } else {
      $('#btn-agregar_integraciones').data('agregar_integraciones', 1);
      $('#btn-agregar_integraciones').text('agregar');
    }
  ...
  $('#btn-agregar_paginas').click(function () {
    if ($(this).data('agregar_paginas') == 1) {
      ...
      $('#btn-agregar_paginas').text('ocultar');
    } else {
      $('#btn-agregar_paginas').data('agregar_paginas', 1);
      $('#btn-agregar_paginas').text('agregar');
    }
  ...
  $('#textarea-politica_envio').summernote({
    placeholder: 'Opcional',
    toolbar: [
      ['style', ['style']],
      ['style', ['bold', 'italic', 'underline', 'clear']],
...
function verSistema(){
  ...
  $.ajax({
    ...
    success: function(response){
      ...
      $('[name="No_Dominio_Externo"]').val(response.No_Dominio_Externo);
      $('[name="Txt_Llave_Externa"]').val(response.Txt_Llave_Externa);
      ...
      $('#cbo-estado_tienda_whatsapp_ver_producto').html('');
      for (var i = 0; i < 2; i++) {
        selected = '';
        if (response.Nu_Estado_Tienda_Whatsapp_Ver_Producto == i)
          selected = 'selected="selected"';
        $('#cbo-estado_tienda_whatsapp_ver_producto').append('<option value="' + i + '" ' + selected + '>' + (i == 1 ? 'Visible' : 'Oculto') + '</option>');
      }

      $('#cbo-contador_item').html('');
      for (var i = 0; i < 2; i++) {
        selected = '';
        if (response.Nu_Estado_Contador_Item_Tienda == i)
          selected = 'selected="selected"';
        $('#cbo-contador_item').append('<option value="' + i + '" ' + selected + '>' + (i == 1 ? 'Visible' : 'Oculto') + '</option>');
      }
      $('[name="Nu_Tiempo_Minutos_Contador_Item_Tienda"]').val(response.Nu_Tiempo_Minutos_Contador_Item_Tienda);
      ...
      $("#textarea-terminos_condiciones").summernote("code", response.Txt_Page_Landing_Terminos);
      ...
      $('#radio-formulario_item_fijo').prop('checked', true).iCheck('update');
      $('#radio-formulario_item_modal').prop('checked', false).iCheck('update');
      if ( response.Nu_Tipo_Fomulario_Item_Tienda == 2 ) {
        $('#radio-formulario_item_fijo').prop('checked', false).iCheck('update');
        $('#radio-formulario_item_modal').prop('checked', true).iCheck('update');
      }

      $('[name="Txt_Titulo_Cabecera_Fomulario_Item_Tienda"]').val(response.Txt_Titulo_Cabecera_Fomulario_Item_Tienda);
      $('[name="Txt_Boton_Fomulario_Item_Tienda"]').val(response.Txt_Boton_Fomulario_Item_Tienda);
      $('[name="Txt_Titulo_Pie_Pagina_Fomulario_Item_Tienda"]').val(response.Txt_Titulo_Pie_Pagina_Fomulario_Item_Tienda);
...
function importarPaginas(sTipo) {
  ...
  $('.modal-title-message-delete').text('¿Deseas importar?');
