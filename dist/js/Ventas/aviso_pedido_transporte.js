  ...
  $( '.btn-proceso_planta_lavanderia' ).click(function(){
    if ( $( '#txt-Filtro_Entidad' ).val().length > 0 && $( '#txt-AID' ).val().length === 0 ) {
      ...
      if ($(this).data('type') == 'html') {
        ...
        url = base_url + 'Ventas/AvisoPedidoTransporteController/sendReportePDF/' + Fe_Inicio + '/' + Fe_Fin + '/' + iIdTipoDocumento + '/' + iIdSerieDocumento + '/' + iNumeroDocumento + '/' + iEstado + '/' + iIdCliente + '/' + sNombreCliente;
        ...
        url = base_url + 'Ventas/AvisoPedidoTransporteController/sendReporteEXCEL/' + Fe_Inicio + '/' + Fe_Fin + '/' + iIdTipoDocumento + '/' + iIdSerieDocumento + '/' + iNumeroDocumento + '/' + iEstado + '/' + iIdCliente + '/' + sNombreCliente;
        window.open(url,'_blank');
...
function getReporteHTML(){
  ...
  url = base_url + 'Ventas/AvisoPedidoTransporteController/sendReporte';
    ...
    if ( response.sStatus == 'success' ) {
      ...
      for (var i = 0; i < iTotalRegistros; i++) {
        ...
        "<tr>"
          ...
          +"<td class='text-center'><span class='label label-" + response[i].No_Class_Estado_Lavado + "'>" + response[i].No_Estado_Lavado + "</span></td>"
        +"</tr>";
...
function finalizarPedido(iIdDocumentoCabecera){
  limpiarValores();

  $( '#hidden-iIdDocumentoCabeceraFinalizado' ).val( iIdDocumentoCabecera );

  $( '#modal-header-label-title_finalizado' ).text( iIdDocumentoCabecera );

  $( '.modal-finalizar_pedido_AvisoPedidoTransporte' ).modal('show');
  $( '.modal-finalizar_pedido_AvisoPedidoTransporte' ).on('shown.bs.modal', function() {
  ...
  $( '#btn-validar_pin_personal_finalizado' ).off('click').click(function () {
    if ( $( '#tel-pin_finalizado' ).val().length===0 ) {
      ...
      scrollToError($('.modal-finalizar_pedido_AvisoPedidoTransporte .modal-body'), $( '#tel-pin_finalizado' ));
    } else {
      $( '.help-block' ).empty();
      ...
      url = base_url + 'Ventas/AvisoPedidoTransporteController/verificarPersonalxPIN';
      var arrParams = {
      ...
      $.post( url, arrParams, function( response ){
        ...
        if ( responsePersonal.sStatus=='success' ) {
          ...
          var arrPost = {
            iIdDocumentoCabecera : iIdDocumentoCabecera,
          };
          $.post( url, arrPost, function( response ){
            if ( response.sStatus == 'success' ) {
...
function generarPedidoFinalizado(){
  ...
  arrCabecera = {
    ...
    'iIdEntidad' : $( '#hidden-iIdEntidadFinalizado' ).val(),
		'sFinalAvisoPedidoTransporte' : $('[name="Txt_AvisoPedidoTransporte"]').val(),
  };
  var $url = base_url + 'Ventas/AvisoPedidoTransporteController/actualizarPedido';
  var $arrParamsPost = {
  ...
  $.post( $url, $arrParamsPost, function( response ) {
    ...
    if ( response.sStatus=='success' ) {
      ...
      $( '.modal-tomar_pedido_AvisoPedidoTransporte' ).modal('hide');
...
function limpiarValores(){
  $( '.modal-tomar_pedido_AvisoPedidoTransporte' ).modal('hide');
  $( '.modal-finalizar_pedido_AvisoPedidoTransporte' ).modal('hide');
  ...
  $( '.div-detalle_item_pedido_finalizado' ).hide();
  $('[name="Txt_AvisoPedidoTransporte"]').val( '' );
  $( '#table-detalle_item_pedido_finalizado' ).hide();
