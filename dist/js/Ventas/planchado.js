...
$(function () {
  ...
  $( '.btn-proceso_planta_lavanderia' ).click(function(){
    if ( $( '#txt-Filtro_Entidad' ).val().length > 0 && $( '#txt-AID' ).val().length === 0 ) {
      ...
      if ($(this).data('type') == 'html') {
        ...
        url = base_url + 'Ventas/PlanchadoController/sendReportePDF/' + Fe_Inicio + '/' + Fe_Fin + '/' + iIdTipoDocumento + '/' + iIdSerieDocumento + '/' + iNumeroDocumento + '/' + iEstado + '/' + iEstadoLavado + '/' + iIdFamilia + '/' + iIdItem + '/' + sNombreItem + '/' + iIdCliente + '/' + sNombreCliente;
        ...
        url = base_url + 'Ventas/PlanchadoController/sendReporteEXCEL/' + Fe_Inicio + '/' + Fe_Fin + '/' + iIdTipoDocumento + '/' + iIdSerieDocumento + '/' + iNumeroDocumento + '/' + iEstado + '/' + iEstadoLavado + '/' + iIdFamilia + '/' + iIdItem + '/' + sNombreItem + '/' + iIdCliente + '/' + sNombreCliente;
        window.open(url,'_blank');
  ...
  $( '#btn-validar_pin_personal_finalizado' ).off('click').click(function () {
    if ( $( '#tel-pin_finalizado' ).val().length===0 ) {
      ...
      scrollToError($('.modal-finalizar_pedido_Planchado .modal-body'), $( '#tel-pin_finalizado' ));
    } else {
      $( '.help-block' ).empty();
      ...
      url = base_url + 'Ventas/PlanchadoController/verificarPersonalxPIN';
      var arrParams = {
  ...
  // /. FINALIZAD PEDIDO
    ...
    if ( $( '#cbo-estado_lavado' ).val() == 0 ) {
      $( '#cbo-estado_lavado' ).closest('.form-group').find('.help-block').html('Seleccionar envío');
      $( '#cbo-estado_lavado' ).closest('.form-group').removeClass('has-success').addClass('has-error');
      
      scrollToError($('.modal-finalizar_pedido_Planchado .modal-body'), $( '#cbo-estado_lavado' ));        
    } else { 
...
function getReporteHTML(){
  ...
  url = base_url + 'Ventas/PlanchadoController/sendReporte';
  $.post( url, arrPost, function( response ){
    if ( response.sStatus == 'success' ) {
      ...
      for (var i = 0; i < iTotalRegistros; i++) {
        ...
        sAccionButton = '';
        if ( response[i].Nu_Estado_Lavado == 7 ){
...
function finalizarPedido(arrParams){
  ...
  $( '.modal-finalizar_pedido_Planchado' ).modal('show');
  $( '.modal-finalizar_pedido_Planchado' ).on('shown.bs.modal', function() {
...
function generarPedidoFinalizado(){
  ...
  arrCabecera = {
    ...
    'sFinalPlanchado' : $('[name="Txt_Planchado"]').val(),
    'iEstadoLavado' : $('#cbo-estado_lavado').val(),
  };
  var $url = base_url + 'Ventas/PlanchadoController/actualizarPedido';
  var $arrParamsPost = {
  ...
  $.post( $url, $arrParamsPost, function( response ) {
    ...
    if ( response.sStatus=='success' ) {
      ...
      $( '.modal-finalizar_pedido_Planchado' ).modal('hide');
    ...
    $( '#btn-finalizar_pedido' ).text('');
    $( '#btn-finalizar_pedido' ).append( 'Finalizar' );
...
function limpiarValores(){
  $( '.modal-tomar_pedido_Planchado' ).modal('hide');
  $( '.modal-finalizar_pedido_Planchado' ).modal('hide');
