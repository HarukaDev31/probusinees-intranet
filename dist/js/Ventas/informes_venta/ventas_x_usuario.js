...
$(function () {
  ...
  $( '.autocompletar_ventas_x_usuario' ).autoComplete({
    minChars: 0,
    source: function (term, response) {
      term = term.trim();
      if (term.length > 2) {
        var term                = term.toLowerCase();
        var global_class_method = $( '.autocompletar_ventas_x_usuario' ).data('global-class_method');
        var global_table        = $( '.autocompletar_ventas_x_usuario' ).data('global-table');
    ...
    renderItem: function (item, search){
      ...
      return '<div title="' + caracteresValidosAutocomplete(item.Nombre) + '" class="autocomplete-suggestion" data-id="' + item.ID + '" data-nombre="' + item.Nombre + '" data-val="' + search + '">' + item.Nombre.replace(re, "<b>$1</b>") + '</div>';
    },
  ...
  $( '.btn-generar_ventas_x_trabajador' ).click(function(){
    ...
    if ($(this).data('type') == 'html') {
      ...
      url = base_url + 'Ventas/informes_venta/VentasXUsuarioController/sendReporte';
      $.post( url, arrPost, function( response ){
        if ( response.sStatus == 'success' ) {
          var iTotalRegistros = response.arrData.length, response=response.arrData, tr_body = '', tr_foot = '';
          var ID_Usuario = '', counter = 0, sum_cantidad = 0.000000, sum_subtotal_s = 0.00, sum_descuento_s = 0.00, sum_igv_s = 0.00, sum_total_s = 0.00, sum_total_d = 0.00;
          ...
          for (var i = 0; i < iTotalRegistros; i++) {
            if (ID_Usuario != response[i].ID_Usuario || $ID_Almacen != response[i].ID_Almacen) {
              if (counter != 0) {
                tr_body +=
                +"<tr>"
              ...
              +"</tr>";
              ID_Usuario = response[i].ID_Usuario;
            }
      ...
      url = base_url + 'Ventas/informes_venta/VentasXUsuarioController/sendReporteEXCEL/' + Fe_Inicio + '/' + Fe_Fin + '/' + ID_Tipo_Documento + '/' + ID_Serie_Documento + '/' + ID_Numero_Documento + '/' + Nu_Estado_Documento + '/' + iIdEmpleado + '/' + encodeURIComponent(sNombreEmpleado) + '/' + iIdItem + '/' + encodeURIComponent(sNombreItem) + '/' + iTipoReporte + '/' + ID_Almacen + '/' + Nu_Tipo_Impuesto + '/' + Nu_Tipo_Venta;
      window.open(url,'_blank');
