...
$(function () {
  ...
  $( '.btn-proceso_planta_lavanderia' ).click(function(){
    if ( $( '#txt-Filtro_Entidad' ).val().length > 0 && $( '#txt-AID' ).val().length === 0 ) {
      ...
      if ($(this).data('type') == 'html') {
        ...
        url = base_url + 'Ventas/PrelavadoController/sendReportePDF/' + Fe_Inicio + '/' + Fe_Fin + '/' + iIdTipoDocumento + '/' + iIdSerieDocumento + '/' + iNumeroDocumento + '/' + iEstado + '/' + iEstadoLavado + '/' + iIdFamilia + '/' + iIdItem + '/' + sNombreItem + '/' + iIdCliente + '/' + sNombreCliente;
        ...
        url = base_url + 'Ventas/PrelavadoController/sendReporteEXCEL/' + Fe_Inicio + '/' + Fe_Fin + '/' + iIdTipoDocumento + '/' + iIdSerieDocumento + '/' + iNumeroDocumento + '/' + iEstado + '/' + iEstadoLavado + '/' + iIdFamilia + '/' + iIdItem + '/' + sNombreItem + '/' + iIdCliente + '/' + sNombreCliente;
        window.open(url,'_blank');
  ...
  $( '#btn-validar_pin_personal' ).off('click').click(function () {
    if ( $( '#tel-pin' ).val().length===0 ) {
      ...
      scrollToError($('.modal-tomar_pedido_prelavado .modal-body'), $( '#tel-pin' ));
    } else {
      $( '.help-block' ).empty();
      ...
      $( '#btn-validar_pin_personal' ).append( 'Ingresando <i class="fa fa-refresh fa-spin fa-lg fa-fw"></i>' );
  
      url = base_url + 'Ventas/PrelavadoController/verificarPersonalxPIN';
      var arrParams = {
        iPin : $( '#tel-pin' ).val(),
      }
      $.post( url, arrParams, function( response ){
        ...
        $( '#btn-validar_pin_personal' ).text('');
        $( '#btn-validar_pin_personal' ).append( 'Entrar' );
  ...
  $( '#btn-validar_pin_personal_finalizado' ).off('click').click(function () {
    if ( $( '#tel-pin_finalizado' ).val().length===0 ) {
      ...
      scrollToError($('.modal-finalizar_pedido_prelavado .modal-body'), $( '#tel-pin_finalizado' ));
    } else {
      $( '.help-block' ).empty();
      ...
      $.post( url, arrParams, function( response ){
        ...
        if ( responsePersonal.sStatus=='success' ) {
          ...
          var arrPost = {
            iIdDocumentoDetalle : iIdDocumentoDetalle,
          };
...
function getReporteHTML(){
  ...
  url = base_url + 'Ventas/PrelavadoController/sendReporte';
  $.post( url, arrPost, function( response ){
    if ( response.sStatus == 'success' ) {
      ...
      for (var i = 0; i < iTotalRegistros; i++) {
        ...
        sAccionButton = '';
        if ( response[i].Nu_Estado_Lavado == 1 || response[i].Nu_Estado_Lavado == 16 ){
          sAccionButton = "<button type='button' class='btn btn-xs btn-link' alt='Tomar pedido' title='Tomar pedido' href='javascript:void(0)' onclick='tomarPedido(" + arrParams + ")'>Tomar pedido</button>";
        } else if ( response[i].Nu_Estado_Lavado == 3 ) {
        ...
        "<tr>"
          ...
          //+"<td class='text-center'>" + response[i].No_Tipo_Pedido_Lavado + "</td>"
...
function tomarPedido(arrParams){
  ...
  iIdDocumentoDetalle = arrParams.iIdDocumentoDetalle;
  sNota = arrParams.sNota;

  limpiarValores();

  $( '#hidden-iIdDocumentoCabecera' ).val( iIdDocumentoCabecera );
  $( '#hidden-iIdDocumentoDetalle' ).val( iIdDocumentoDetalle );
  $( '#modal-header-label-title_tomar' ).text( arrParams.sTipoDocumento + ' - ' + arrParams.sSerieDocumento + ' - ' + arrParams.sNumeroDocumento);
  ...
  if ( sNota != null && sNota.length>0 ) {
    ...
    $( '#modal-header-label-subtitle_nota' ).text( 'Nota de caja: ' + clearHTMLTextArea(sNota) );
  }

  $( '.modal-tomar_pedido_prelavado' ).modal('show');
  $( '.modal-tomar_pedido_prelavado' ).on('shown.bs.modal', function() {
    $( '#tel-pin' ).focus();
  });
...
function finalizarPedido(arrParams){
  ...
  $( '#hidden-iIdDocumentoCabeceraFinalizado' ).val( iIdDocumentoCabecera );
  $( '#hidden-iIdDocumentoDetalleFinalizado' ).val( iIdDocumentoDetalle );
  $( '#hidden-iEstadoLavado' ).val( iEstadoLavado );

  $( '#modal-header-label-title_finalizado' ).text( arrParams.sTipoDocumento + ' - ' + arrParams.sSerieDocumento + ' - ' + arrParams.sNumeroDocumento);

  $( '#h4-datos_item_finalizado_prelavado' ).text( 'Cantidad: ' + arrParams.fCantidadItem + ' - Item: ' + arrParams.sNombreItem );

  $( '.modal-header-label-subtitle_nota_finalizado' ).hide();
  if ( sNota != null && sNota.length>0 ) {
    $( '.modal-header-label-subtitle_nota_finalizado' ).show();
    $( '#modal-header-label-subtitle_nota_finalizado' ).text( 'Nota de caja: ' + sNota );
  }

  $( '.modal-finalizar_pedido_prelavado' ).modal('show');
  $( '.modal-finalizar_pedido_prelavado' ).on('shown.bs.modal', function() {
...
function generarPedido(){
  ...
  arrCabecera = {
    ...
    'iIdDocumentoDetalle' : $( '#hidden-iIdDocumentoDetalle' ).val(),
    'iIdEntidad' : $( '#hidden-iIdEntidad' ).val(),
  };

  var $url = base_url + 'Ventas/PrelavadoController/agregarPedido';
  var $arrParamsPost = {
  ...
  $.post( $url, $arrParamsPost, function( response ) {
    ...
    if ( response.sStatus=='success' ) {
      ...
      $( '.modal-tomar_pedido_prelavado' ).modal('hide');
    ...
    $( '#btn-procesar_pedido' ).text('');
    $( '#btn-procesar_pedido' ).append( 'Entrar' );
...
function generarPedidoFinalizado(){
  ...
  arrCabecera = {
    ...
    'iEstadoLavado' : $( '#hidden-iEstadoLavado' ).val(),
    'iEstadoLavadoCombobox' : $( '#cbo-estado_lavado' ).val(),
		'sFinalPrelavado' : $('[name="Txt_Final_Prelavado"]').val(),
  };
  var $url = base_url + 'Ventas/PrelavadoController/actualizarPedido';
  var $arrParamsPost = {
    arrCabecera : arrCabecera,
  };
  ...
  $.post( $url, $arrParamsPost, function( response ) {
    ...
    $( '#btn-finalizar_pedido' ).text('');
    $( '#btn-finalizar_pedido' ).append( 'Finalizar pedido' );
  ...
  .fail(function(jqXHR, textStatus, errorThrown) {
    ...
    $( '#btn-finalizar_pedido' ).text('');
    $( '#btn-finalizar_pedido' ).attr('disabled', false);
    $( '#btn-finalizar_pedido' ).append( 'Entrar' );
    $( '#btn-salir' ).attr('disabled', false);
  })
}
...
function limpiarValores(){
  $( '.modal-tomar_pedido_prelavado' ).modal('hide');
  $( '.modal-finalizar_pedido_prelavado' ).modal('hide');

  $( '#div-pin' ).show();
  $( '#hidden-iIdDocumentoCabecera' ).val( '' );
  $( '#hidden-iIdDocumentoDetalle' ).val( '' );
  ...
  $( '#hidden-iIdDocumentoDetalleFinalizado' ).val( '' );
  $( '#hidden-iEstadoLavado' ).val( '' );
  $( '#hidden-iIdEntidadFinalizado' ).val( '' );
  $( '.div-detalle_item_pedido_finalizado' ).hide();
  $( '.div-estado_lavado' ).hide();
  $('[name="Txt_Final_Prelavado"]').val( '' );
  $( '#table-detalle_item_pedido_finalizado' ).hide();
  $( '#table-detalle_item_pedido_finalizado tbody' ).empty();
