...
$(function () {  
  ...
  $( '.btn-saldo_proveedor' ).click(function(){
    if ( $( '#txt-Filtro_Entidad' ).val().length > 0 && $( '#txt-AID' ).val().length === 0 ) {
      $( '#txt-Filtro_Entidad' ).closest('.form-group').find('.help-block').html('Seleccionar proveedor');
		  $( '#txt-Filtro_Entidad' ).closest('.form-group').removeClass('has-success').addClass('has-error');
    } else {
      $( '.help-block' ).empty();
    
      var ID_Almacen, Fe_Inicio, Fe_Fin, iIdTipoDocumento, iIdSerieDocumento, iNumeroDocumento, iEstadoPago, iIdProveedor, sNombreProveedor;
      ...
      ID_Almacen = $('#cbo-Almacenes_Saldo_x_Proveedor').val();
      ...
      if ($(this).data('type') == 'html') {
        $( '#btn-html_saldo_proveedor' ).text('');
        $( '#btn-html_saldo_proveedor' ).attr('disabled', true);
        $( '#btn-html_saldo_proveedor' ).append( 'Cargando <i class="fa fa-refresh fa-spin fa-lg fa-fw"></i>' );
    
        $( '#table-saldo_proveedor > tbody' ).empty();
        $( '#table-saldo_proveedor > tfoot' ).empty();
        
        url = base_url + 'Logistica/informes_logistica/SaldoProveedorController/sendReporte';
        $.post( url, arrPost, function( response ){
          ...
          $( '#div-saldo_proveedor' ).show();
          $( '#table-saldo_proveedor > tbody' ).append(tr_body);
          $( '#table-saldo_proveedor > tbody' ).after(tr_foot);
          
          $( '#btn-html_saldo_proveedor' ).text('');
          $( '#btn-html_saldo_proveedor' ).append( '<i class="fa fa-search"></i> Buscar' );
          $( '#btn-html_saldo_proveedor' ).attr('disabled', false);
        }, 'JSON')
        ...
        $( '#btn-pdf_saldo_proveedor' ).text('');
        $( '#btn-pdf_saldo_proveedor' ).attr('disabled', true);
        $( '#btn-pdf_saldo_proveedor' ).append( 'Cargando <i class="fa fa-refresh fa-spin fa-lg fa-fw"></i>' );

        url = base_url + 'Logistica/informes_logistica/SaldoProveedorController/sendReportePDF/' + Fe_Inicio + '/' + Fe_Fin + '/' + iIdTipoDocumento + '/' + iIdSerieDocumento + '/' + iNumeroDocumento + '/' + iEstadoPago + '/' + iIdProveedor + '/' + sNombreProveedor + '/' + ID_Almacen;
        window.open(url,'_blank');
        
        $( '#btn-pdf_saldo_proveedor' ).text('');
        $( '#btn-pdf_saldo_proveedor' ).append( '<i class="fa fa-file-pdf-o color_icon_pdf"></i> PDF' );
        $( '#btn-pdf_saldo_proveedor' ).attr('disabled', false);
      } else if ($(this).data('type') == 'excel') {
        $( '#btn-excel_saldo_proveedor' ).text('');
        $( '#btn-excel_saldo_proveedor' ).attr('disabled', true);
        $( '#btn-excel_saldo_proveedor' ).append( 'Cargando <i class="fa fa-refresh fa-spin fa-lg fa-fw"></i>' );
        
        url = base_url + 'Logistica/informes_logistica/SaldoProveedorController/sendReporteEXCEL/' + Fe_Inicio + '/' + Fe_Fin + '/' + iIdTipoDocumento + '/' + iIdSerieDocumento + '/' + iNumeroDocumento + '/' + iEstadoPago + '/' + iIdProveedor + '/' + sNombreProveedor + '/' + ID_Almacen;
        window.open(url,'_blank');
        
        $( '#btn-excel_saldo_proveedor' ).text('');
        $( '#btn-excel_saldo_proveedor' ).append( '<i class="fa fa-file-excel-o color_icon_excel"></i> Excel' );
        $( '#btn-excel_saldo_proveedor' ).attr('disabled', false);
      }// /. if all button 
    }// /. if - else validacion
  })// /. btn
})
...
function getAlmacenes(arrParams) {
  $('#cbo-Almacenes_Saldo_x_Proveedor').html('<option value="0">- Todos -</option>');
  ...
  $.post(url, {}, function (responseAlmacen) {
    ...
    if (iCantidadRegistros == 1) {
      ...
      $('#cbo-Almacenes_Saldo_x_Proveedor').append('<option value="' + responseAlmacen[0]['ID_Almacen'] + '" ' + selected + ' data-direccion_almacen="' + responseAlmacen[0]['Txt_Direccion_Almacen'] + '">' + responseAlmacen[0]['No_Almacen'] + '</option>');
    } else {
      for (var i = 0; i < iCantidadRegistros; i++) {
        ...
        $('#cbo-Almacenes_Saldo_x_Proveedor').append('<option value="' + responseAlmacen[i]['ID_Almacen'] + '" ' + selected + ' data-direccion_almacen="' + responseAlmacen[0]['Txt_Direccion_Almacen'] + '">' + responseAlmacen[i]['No_Almacen'] + '</option>');
      }
    }
    $('#modal-loader').modal('hide');
  }, 'JSON');
