...
$(function () {
  ...
  $('#btn-save_delivery_importe').off('click').click(function () {
    ...
    if($( '#txt-modal-precio_delivery' ).val().length==0){
      ...
      */
      ...
      url = base_url + 'CallCenterController/actualizarPrecioDelivery';
      $.ajax({
        type		  : 'POST',
        dataType	: 'JSON',
        url		    : url,
  ...
  $('#btn-save_nota_pedido').off('click').click(function () {
    if($('[name="Txt_Glosa_Pedido"]').val().length==0){
      ...
      url = base_url + 'CallCenterController/actualizarNotaPedido';
      $.ajax({
        type		  : 'POST',
        dataType	: 'JSON',
        url		    : url,
  ...
  $('#btn-save_asignar_delivery').off('click').click(function () {
    if($('[name="ID_Usuario_Asignar_Delivery"]').val().length==0){
      ...
      url = base_url + 'CallCenterController/asignarUsuarioDelivery';
      $.ajax({
        type		  : 'POST',
        dataType	: 'JSON',
        url		    : url,
  ...
  //cambiar precio delivery
  $('#btn-save_delivery_saldo_a_favor').off('click').click(function () {
    /*
    if($( '#txt-modal-precio_delivery_saldo_a_favor' ).val().length==0){
      $( '#txt-modal-precio_delivery_saldo_a_favor' ).closest('.form-group').find('.help-block').html('Ingresar precio');
      $( '#txt-modal-precio_delivery_saldo_a_favor' ).closest('.form-group').removeClass('has-success').addClass('has-error');
    } else if(parseFloat($( '#txt-modal-precio_delivery_saldo_a_favor' ).val())<=0.00 || isNaN(parseFloat($( '#txt-modal-precio_delivery_saldo_a_favor' ).val()))){
      $( '#txt-modal-precio_delivery_saldo_a_favor' ).closest('.form-group').find('.help-block').html('Ingresar precio');
      $( '#txt-modal-precio_delivery_saldo_a_favor' ).closest('.form-group').removeClass('has-success').addClass('has-error');
    } else {
      */
      $( '#btn-save_delivery_saldo_a_favor' ).text('');
      $( '#btn-save_delivery_saldo_a_favor' ).attr('disabled', true);
      $( '#btn-save_delivery_saldo_a_favor' ).append( 'Guardando <i class="fa fa-refresh fa-spin fa-lg fa-fw"></i>' );

      var arrDataDelivery = Array();
      arrDataDelivery = {
        'id_pedido_cabecera' : $( '#hidden-modal-id_pedido_cabecera_saldo_a_favor' ).val(),
        'precio_delivery' : $( '#txt-modal-precio_delivery_saldo_a_favor' ).val()
      }

      url = base_url + 'CallCenterController/actualizarPrecioDeliverySaldoAFavor';
      $.ajax({
        type		  : 'POST',
        dataType	: 'JSON',
        url		    : url,
        ...
        success: function (response) {
          $('.modal-delivery_saldo_a_favor').modal('hide');
          ...
          $( '#btn-save_delivery_saldo_a_favor' ).text('');
          $( '#btn-save_delivery_saldo_a_favor' ).append( 'Guardar' );
          $( '#btn-save_delivery_saldo_a_favor' ).attr('disabled', false);
        },
        error: function (jqXHR, textStatus, errorThrown) {
          $( '.modal-message' ).removeClass('modal-danger modal-warning modal-success');
  ...
  $( '.btn-generar_ventas_detalladas_generales' ).click(function(){
    var Fe_Inicio, Fe_Fin, ID_Tipo_Documento, ID_Pedido_Cabecera, Nu_Estado_Pedido, iIdCliente, sNombreCliente, iIdItem, sNombreItem, iTipoVenta, ID_Filtro_Empresa, Nu_Estado_Pedido_Empresa, sNombreCiudad, ID_Pais;
    ...
    if ($(this).data('type') == 'html') {
      ...
      url = base_url + 'CallCenterController/sendReportePDF/' + Fe_Inicio + '/' + Fe_Fin + '/' + ID_Tipo_Documento + '/' + ID_Pedido_Cabecera + '/' + Nu_Estado_Pedido + '/' + iIdCliente + '/' + encodeURIComponent(sNombreCliente) + '/' + ID_Filtro_Empresa + '/' + Nu_Estado_Pedido_Empresa + '/' + encodeURIComponent(sNombreCiudad);
      ...
      url = base_url + 'CallCenterController/sendReporteEXCEL/' + Fe_Inicio + '/' + Fe_Fin + '/' + ID_Tipo_Documento + '/' + ID_Pedido_Cabecera + '/' + Nu_Estado_Pedido + '/' + iIdCliente + '/' + encodeURIComponent(sNombreCliente) + '/' + ID_Filtro_Empresa + '/' + Nu_Estado_Pedido_Empresa + '/' + encodeURIComponent(sNombreCiudad) + '/' + ID_Pais;
      window.open(url,'_blank');
  ...
  $('#btn-modal-generar_venta-send').click(function () {
    if (parseFloat($('[name="fTotalDocumento"]').val()) < 0.10) {
      ...
      url = base_url + 'CallCenterController/generarVenta';
      $.ajax({
        type: 'POST',
        dataType: 'JSON',
        url: url,
...
function getReporteHTML() {
  ...
  url = base_url + 'CallCenterController/sendReporte';
    ...
    if (response.sStatus == 'success') {
      ...
      for (var i = 0; i < iTotalRegistros; i++) {
        ...
        "<tr>"
          ...
          +"<td class='text-right'>" + response[i].Ss_Saldo_A_Favor_Delivery_Interno + "</td>"
          +"<td class='text-right'>" + response[i].Ss_Precio_Delivery_Propio_Personal + "</td>"
...
function verPedido(iIdPedido){
  ...
  url = base_url + 'CallCenterController/verPedido/' + iIdPedido;
...
function cambiarEstado(ID, Nu_Estado, ID_Pais, sNombreClase, sNombreEstado) {
  
  console.log('sNombreClase > ' + sNombreClase);
  console.log('sNombreEstado > ' + sNombreEstado);

  console.log( $('#btn-dropdown-pedido_estado-' + ID).text() );
  console.log( $('#btn-dropdown-pedido_estado-' + ID).attr('class') );
  ...
  $('#btn-save-delete').off('click').click(function () {
    $('#modal-loader').modal('show');
*/
    url = base_url + 'CallCenterController/cambiarEstado/' + ID + '/' + Nu_Estado + '/' + ID_Pais;
    $.ajax({
      url: url,
      type: "GET",
      dataType: "JSON",
      success: function (response) {
        ...
        */
        ...
        if (response.status == 'success') {
          $('#btn-dropdown-pedido_estado-' + ID).removeClass($('#btn-dropdown-pedido_estado-' + ID).attr('class'));

          $('#btn-dropdown-pedido_estado-' + ID).html( sNombreEstado + ' <span class="caret"></span>' );
          $('#btn-dropdown-pedido_estado-' + ID).addClass('btn dropdown-toggle btn-' + sNombreClase);
...
function eliminarPedido(ID) {
  ...
  $('#btn-save-delete').off('click').click(function () {
    ...
    url = base_url + 'CallCenterController/eliminarPedido/' + ID;
    $.ajax({
      url: url,
      type: "GET",
      dataType: "JSON",
...
function generarVenta(ID_Pedido_Cabecera){
  ...
  $('#modal-loader').modal('show');
  url = base_url + 'CallCenterController/verPedido/' + ID_Pedido_Cabecera;
  $.getJSON(url, function (response) {
    if (response.sStatus == 'success') {
...
function form_AgregarPedido(){
  ...
  if ( $( '#txt-EID_Pedido_Cabecera' ).val().length==0 && arrDetallePedido.length === 0){
    ...
    url = base_url + 'CallCenterController/crudPedido';
    $.ajax({
      type		  : 'POST',
      dataType	: 'JSON',
      url		    : url,
...
function editarPedido(ID){
  ...
  url = base_url + 'CallCenterController/ajax_edit/' + ID;
  $.ajax({
    url : url,
    type: "GET",
    dataType: "JSON",
    success: function(response){
      ...
      for (i = 0; i < response.length; i++) {
        ...
        "<tr id='tr_enlace_producto" + response[i]['ID_Producto'] + "'>"
          ...
          + "<td class='text-right text-precio'>" + number_format(response[i]['Ss_Total'], 2) + "</td>"
...
function cambiarEstadoPedidoEmpresa(ID, Nu_Estado) {
  ...
  $('#btn-save-delete').off('click').click(function () {
    ...
    url = base_url + 'CallCenterController/cambiarEstadoPedidoEmpresa/' + ID + '/' + Nu_Estado;
    $.ajax({
      url: url,
      type: "GET",
      dataType: "JSON",
...
function agregarSaldoFavor(ID){
  $( '.form-group' ).removeClass('has-error');
  $( '.form-group' ).removeClass('has-success');
  $( '.help-block' ).empty();

  $( '#hidden-modal-id_pedido_cabecera_saldo_a_favor' ).val(ID);
	$( '.modal-delivery_saldo_a_favor' ).modal('show');
  $( '#txt-modal-precio_delivery_saldo_a_favor' ).val('');
  $( '.modal-delivery_saldo_a_favor' ).on('shown.bs.modal', function() {
    $( '#txt-modal-precio_delivery_saldo_a_favor' ).focus();
  })
}

function generarGuia99(ID){
  var $modal_delete = $('#modal-message-delete');
  $modal_delete.modal('show');
  ...
  $('.modal-title-message-delete').text('¿Generar Guia de 99 Minutos?');
  ...
  $('#btn-save-delete').off('click').click(function () {
    ...
    url = base_url + 'CallCenterController/generarGuia99/' + ID;
    $.ajax({
      url: url,
      type: "GET",
      dataType: "JSON",
...
function generarGuiaQuiken(ID){
  ...
  $('.modal-title-message-delete').text('¿Generar Guia de Quiken?');
  ...
  $('#btn-save-delete').off('click').click(function () {
    ...
    url = base_url + 'CallCenterController/generarGuiaQuiken/' + ID;
    $.ajax({
      url: url,
      type: "GET",
      dataType: "JSON",
...
function generarGuiaEcxlae(ID){
  ...
  $('.modal-title-message-delete').text('¿Generar Guia de Ecxlae?');
  ...
  $('#btn-save-delete').off('click').click(function () {
    ...
    url = base_url + 'CallCenterController/generarGuiaEcxlae/' + ID;
    $.ajax({
      url: url,
      type: "GET",
      dataType: "JSON",
