...
$(function () {
  ...
  $('#btn-save_delivery_importe').off('click').click(function () {
    if($( '#txt-modal-precio_delivery' ).val().length==0){
      ...
      url = base_url + 'Proveedores/PedidoProveedoDropshippingController/actualizarPrecioDelivery';
      $.ajax({
        type		  : 'POST',
        dataType	: 'JSON',
        url		    : url,
  ...
  $('#btn-save_nota_pedido').off('click').click(function () {
    if($('[name="Txt_Glosa_Pedido"]').val().length==0){
      $('[name="Txt_Glosa_Pedido"]').closest('.form-group').find('.help-block').html('Ingresar nota');
      $('[name="Txt_Glosa_Pedido"]').closest('.form-group').removeClass('has-success').addClass('has-error');
    } else {
      ...
      arrDataNotaPedido = {
        ...
        'nota' : $( '[name="Txt_Glosa_Pedido"]' ).val()
      }

      url = base_url + 'Proveedores/PedidoProveedoDropshippingController/actualizarNotaPedido';
      $.ajax({
        type		  : 'POST',
        dataType	: 'JSON',
        url		    : url,
  ...
  //ASIGNAR DELIVERY
  $('#btn-save_asignar_delivery').off('click').click(function () {
    if($('[name="ID_Usuario_Asignar_Delivery"]').val().length==0){
      $('[name="ID_Usuario_Asignar_Delivery"]').closest('.form-group').find('.help-block').html('Seleccionar Delivery');
      $('[name="ID_Usuario_Asignar_Delivery"]').closest('.form-group').removeClass('has-success').addClass('has-error');
    } else {
      $( '#btn-save_asignar_delivery' ).text('');
      $( '#btn-save_asignar_delivery' ).attr('disabled', true);
      $( '#btn-save_asignar_delivery' ).append( 'Guardando <i class="fa fa-refresh fa-spin fa-lg fa-fw"></i>' );

      var arrDataNotaPedido = Array();
      arrDataNotaPedido = {
        'id_pedido_cabecera' : $( '#hidden-modal-id_pedido_cabecera_delivery' ).val(),
        'id_usuario_delivery' : $( '[name="ID_Usuario_Asignar_Delivery"]' ).val()
      }

      url = base_url + 'Proveedores/PedidoProveedoDropshippingController/asignarUsuarioDelivery';
      $.ajax({
        type		  : 'POST',
        dataType	: 'JSON',
        url		    : url,
        ...
        success: function (response) {
          $('.modal-asignar_delivery').modal('hide');
          ...
          $( '#btn-save_asignar_delivery' ).text('');
          $( '#btn-save_asignar_delivery' ).append( 'Guardar' );
          $( '#btn-save_asignar_delivery' ).attr('disabled', false);
        },
        error: function (jqXHR, textStatus, errorThrown) {
          $( '.modal-message' ).removeClass('modal-danger modal-warning modal-success');
  ...
  //FILTRO SOLO PROVEEDOR
  url = base_url + 'HelperDropshippingController/getProveedoresDropshippingSinPais';
  var selected = '';
  ...
  $( '.btn-generar_ventas_detalladas_generales' ).click(function(){
    ...
    if ($(this).data('type') == 'html') {
      ...
      url = base_url + 'Proveedores/PedidoProveedoDropshippingController/sendReportePDF/' + Fe_Inicio + '/' + Fe_Fin + '/' + ID_Tipo_Documento + '/' + ID_Pedido_Cabecera + '/' + Nu_Estado_Pedido + '/' + iIdCliente + '/' + encodeURIComponent(sNombreCliente) + '/' + ID_Filtro_Empresa + '/' + Nu_Estado_Pedido_Empresa;
      ...
      url = base_url + 'Proveedores/PedidoProveedoDropshippingController/sendReporteEXCEL/' + Fe_Inicio + '/' + Fe_Fin + '/' + ID_Tipo_Documento + '/' + ID_Pedido_Cabecera + '/' + Nu_Estado_Pedido + '/' + iIdCliente + '/' + encodeURIComponent(sNombreCliente) + '/' + ID_Filtro_Empresa + '/' + Nu_Estado_Pedido_Empresa;
      window.open(url,'_blank');
  ...
  $('#btn-modal-generar_venta-send').click(function () {
    if (parseFloat($('[name="fTotalDocumento"]').val()) < 0.10) {
      ...
      url = base_url + 'Proveedores/PedidoProveedoDropshippingController/generarVenta';
      $.ajax({
        type: 'POST',
        dataType: 'JSON',
        url: url,
...
function getReporteHTML() {
  ...
  url = base_url + 'Proveedores/PedidoProveedoDropshippingController/sendReporte';
  $.post(url, arrPost, function (response) {
    if (response.sStatus == 'success') {
      var iTotalRegistros = response.arrData.length, response = response.arrData, tr_body = '', tr_foot = '', fTotalItem = 0.00, fTotalGeneral = 0.00, fPrecioTotalPedioEcxlae=0.00, fTotalPrecioGeneral = 0.00, fPrecioTotalPedidoProveedor=0.00, fTotalPagarGeneral=0.00, fTotalGanancia=0;
      for (var i = 0; i < iTotalRegistros; i++) {
        fTotal = (!isNaN(parseFloat(response[i].Ss_Total)) ? parseFloat(response[i].Ss_Total) : 0);
        fPrecioTotalPedioEcxlae = (!isNaN(parseFloat(response[i].fPrecioTotalPedioEcxlae)) ? parseFloat(response[i].fPrecioTotalPedioEcxlae) : 0);
        fPrecioTotalPedidoProveedor = (!isNaN(parseFloat(response[i].fPrecioTotalPedidoProveedor)) ? parseFloat(response[i].fPrecioTotalPedidoProveedor) : 0);
        fTotalGanancia = (!isNaN(parseFloat(response[i].fTotalGanancia)) ? parseFloat(response[i].fTotalGanancia) : 0);
        ...
        "<tr>"
          ...
          +"<td class='text-right'>" + number_format(fPrecioTotalPedidoProveedor, 2) + "</td>"
          +"<td class='text-right'>" + number_format(fPrecioTotalPedioEcxlae, 2) + "</td>"
          +"<td class='text-right'>" + number_format(fTotalGanancia, 2) + "</td>"
          ...
          //+"<td class='text-center'><span class='label label-" + sClassStatusPedidoEmpresa + "'>" + response[i].sEstadoPedidoEmpresa + "</span></td>"
          ...
          //+"<td class='text-center'>" + response[i].Ss_Precio_Delivery_Propio_Personal + "</td>"
        ...
        fTotalGeneral += fPrecioTotalPedidoProveedor;
        fTotalPrecioGeneral += fPrecioTotalPedioEcxlae;
        fTotalPagarGeneral += fTotalGanancia;
        //fTotalPagarGeneral += (fPrecioTotalPedidoProveedor - fPrecioTotalPedioEcxlae);
      }
      ...
      tr_foot =
        ...
        + "<th class='text-right'>" + number_format(fTotalPrecioGeneral, 2) + "</th>"
        + "<th class='text-right'>" + number_format(fTotalPagarGeneral, 2) + "</th>"
...
function verPedido(iIdPedido){
  ...
  url = base_url + 'Proveedores/PedidoProveedoDropshippingController/verPedido/' + iIdPedido;
...
function cambiarEstado(ID, Nu_Estado) {
  ...
  $('#btn-save-delete').off('click').click(function () {
    ...
    url = base_url + 'Proveedores/PedidoProveedoDropshippingController/cambiarEstado/' + ID + '/' + Nu_Estado;
    $.ajax({
      url: url,
      type: "GET",
      dataType: "JSON",
...
function eliminarPedido(ID) {
  ...
  $('#btn-save-delete').off('click').click(function () {
    ...
    url = base_url + 'Proveedores/PedidoProveedoDropshippingController/eliminarPedido/' + ID;
    $.ajax({
      url: url,
      type: "GET",
      dataType: "JSON",
...
function generarVenta(ID_Pedido_Cabecera){
  ...
  $('#modal-loader').modal('show');
  url = base_url + 'Proveedores/PedidoProveedoDropshippingController/verPedido/' + ID_Pedido_Cabecera;
  $.getJSON(url, function (response) {
    if (response.sStatus == 'success') {
...
function form_AgregarPedido(){
  ...
  if ( $( '#txt-EID_Pedido_Cabecera' ).val().length==0 && arrDetallePedido.length === 0){
    ...
    url = base_url + 'Proveedores/PedidoProveedoDropshippingController/crudPedido';
    $.ajax({
      type		  : 'POST',
      dataType	: 'JSON',
      url		    : url,
...
function editarPedido(ID){
  ...
  url = base_url + 'Proveedores/PedidoProveedoDropshippingController/ajax_edit/' + ID;
  $.ajax({
    url : url,
    type: "GET",
    dataType: "JSON",
    success: function(response){
      ...
      for (i = 0; i < response.length; i++) {
        ...
        "<tr id='tr_enlace_producto" + response[i]['ID_Producto'] + "'>"
          ...
          + "<td class='text-right text-precio'>" + number_format(response[i]['Ss_Precio_Empresa_Proveedor'], 2) + "</td>";
          if( response[i]['ID_Pais']==1 ){//1=peru
            table_enlace_producto += "<td class='text-right text-precio'>" + number_format(3, 2) + "</td>";
          } else if( response[i]['ID_Pais']==2 ){//2=mexico
            table_enlace_producto += "<td class='text-right text-precio'>" + number_format(20, 2) + "</td>";
          }
          //+ "<td class='text-center'><button type='button' id='btn-deleteProductoEnlace' class='btn btn-xs btn-link' alt='Quitar' title='Quitar'><i class='fa fa-trash-o fa-2x' aria-hidden='true'></i></button></td>"
...
function cambiarEstadoPedidoEmpresa(ID, Nu_Estado) {
  ...
  $('#btn-save-delete').off('click').click(function () {
    ...
    url = base_url + 'Proveedores/PedidoProveedoDropshippingController/cambiarEstadoPedidoEmpresa/' + ID + '/' + Nu_Estado;
    $.ajax({
      url: url,
      type: "GET",
      dataType: "JSON",
...
function agregarNotaPedido(ID){
  ...
  $('[name="Txt_Glosa_Pedido"]').val('');
  $( '.modal-nota_pedido' ).on('shown.bs.modal', function() {
    $( '[name="Txt_Glosa_Pedido"]' ).focus();
  })
}
...
function asignarDelivery(ID){
  ...
  $( '#hidden-modal-id_pedido_cabecera_delivery' ).val(ID);
	$( '.modal-asignar_delivery' ).modal('show');
  
  //listar banco
  $('#cbo-delivery').html('<option value="" selected="selected">- Cargando -</option>');
  url = base_url + 'HelperDropshippingController/listarUsuarioDelivery';
  $.post(url, {}, function (response) {
    if (response.status == 'success') {
      var l = response.result.length;
      $('#cbo-delivery').html('<option value="" selected="selected">- Seleccionar -</option>');
      for (var x = 0; x < l; x++) {
        $('#cbo-delivery').append('<option value="' + response.result[x].ID_Usuario + '">' + response.result[x].No_Nombres_Apellidos + '</option>');
      }
    } else {
      if (response.sMessageSQL !== undefined) {
        console.log(response.sMessageSQL);
...
function pdfGuia99(ID, ID_Pedido_Cabecera, imprimir_guia_mexico, Nu_Estado){
  var $modal_delete = $( '#modal-message-delete' );
  $modal_delete.modal('show');
  ...
  $( '#btn-save-delete' ).off('click').click(function () {
    if(imprimir_guia_mexico==0 && Nu_Estado==8){
      actualizarEstadoPreparandoMexico(ID_Pedido_Cabecera, imprimir_guia_mexico, Nu_Estado);
    }
...
function actualizarEstadoPreparandoMexico(ID_Pedido_Cabecera, imprimir_guia_mexico, Nu_Estado){
  console.log('Cambiando estado pedido: ' + ID_Pedido_Cabecera);
  console.log('Estado de impresion: ' + imprimir_guia_mexico);
    
  var sendPost = {
    ID: ID_Pedido_Cabecera,
    imprimir_guia_mexico: imprimir_guia_mexico,
    Nu_Estado: Nu_Estado
  };

  url = base_url + 'Proveedores/PedidoProveedoDropshippingController/cambiarEstadoImpresionGuiaMexico';
  $.ajax({
    url: url,
    type: "POST",
    dataType: "JSON",
...
function sendPDF($modal_delete, ID){
  console.log('Impresión de pedido: ' + ID);
...
function pdfQuiken(URL_PDF, ID_Pedido_Cabecera, imprimir_guia_mexico, Nu_Estado){
  var $modal_delete = $( '#modal-message-delete' );
  $modal_delete.modal('show');
...
function pdfGuiaEcxlae(ID, ID_Pedido_Cabecera, imprimir_guia_mexico, Nu_Estado){
  var $modal_delete = $( '#modal-message-delete' );
  $modal_delete.modal('show');
