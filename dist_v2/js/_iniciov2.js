$(function () {
  ...
  */ 
  ...
  $("#form-documento_proveedor_exportacion").on('submit',function(e){
	...
	$(document).on('click', '#btn-save_oc_reservar_pedido', function (e) {
    e.preventDefault();
    $( '#btn-save_oc_reservar_pedido' ).text('');
    $( '#btn-save_oc_reservar_pedido' ).attr('disabled', true);
    $( '#btn-save_oc_reservar_pedido' ).append( 'Guardando <i class="fa fa-refresh fa-spin fa-lg fa-fw"></i>' );

    //$( '#modal-loader' ).modal('show');

    url = base_url + 'AgenteCompra/PedidosPagados/reservaPedido';
      $.ajax({
      type		  : 'POST',
      dataType	: 'JSON',
      url		    : url,
      data		  : $('#form-oc_reservar_pedido').serialize(),
      success : function( response ){
        //$( '#modal-loader' ).modal('hide');
        ...
        if (response.status == 'success'){
          $('.modal-oc_reservar_pedido').modal('hide');
        ...
        $( '#btn-save_oc_reservar_pedido' ).text('');
        $( '#btn-save_oc_reservar_pedido' ).append( 'Guardar' );
        $( '#btn-save_oc_reservar_pedido' ).attr('disabled', false);
      },
      error: function (jqXHR, textStatus, errorThrown) {
        //$( '#modal-loader' ).modal('hide');
        $('#moda-message-content').removeClass('bg-danger bg-warning bg-success');
...
function reservarPedidoOC(id, sCorrelativo){
  $( '[name="oc_reservar_pedido-ID_Pedido_Cabecera"]' ).val(id);

  $( '.modal-oc_reservar_pedido' ).modal('show');
  $( '#form-oc_reservar_pedido' )[0].reset();
  ...
  $.ajax({
    ...
    dataType: "JSON",
    success: function(response){
      console.log(response.Nu_Tipo_Servicio);
      console.log(response.Nu_Tipo_Incoterms);
      console.log(response.Nu_Tipo_Transporte_Maritimo);

      var selected = '';
      $('#oc_reservar_pedido-Nu_Tipo_Servicio').html('<option value="0">- Seleccionar -</option>');
      $('#oc_reservar_pedido-Nu_Tipo_Incoterms').html('<option value="0">- Seleccionar -</option>');
      $('#oc_reservar_pedido-Nu_Tipo_Transporte_Maritimo').html('<option value="0">- Seleccionar -</option>');
      
      selected = '';
      if (response.Nu_Tipo_Servicio == '1')
        selected = 'selected="selected"';
      $('#oc_reservar_pedido-Nu_Tipo_Servicio').append('<option value="1" ' + selected + '>Trading</option>');
      
      selected = '';
      if (response.Nu_Tipo_Servicio == '2')//c.trading
        selected = 'selected="selected"';
      $('#oc_reservar_pedido-Nu_Tipo_Servicio').append('<option value="2" ' + selected + '>C. Trading</option>');
      
      selected = '';
      if (response.Nu_Tipo_Incoterms == '1')
        selected = 'selected="selected"';
      $('#oc_reservar_pedido-Nu_Tipo_Incoterms').append('<option value="1" ' + selected + '>EXW</option>');
      
      selected = '';
      if (response.Nu_Tipo_Incoterms == '2')//c.trading
        selected = 'selected="selected"';
      $('#oc_reservar_pedido-Nu_Tipo_Incoterms').append('<option value="2" ' + selected + '>FOB</option>');
      
      selected = '';
      if (response.Nu_Tipo_Incoterms == '3')//c.trading
        selected = 'selected="selected"';
      $('#oc_reservar_pedido-Nu_Tipo_Incoterms').append('<option value="3" ' + selected + '>CIF</option>');
      
      selected = '';
      if (response.Nu_Tipo_Incoterms == '4')//c.trading
        selected = 'selected="selected"';
      $('#oc_reservar_pedido-Nu_Tipo_Incoterms').append('<option value="4" ' + selected + '>DDP</option>');
      
      selected = '';
      if (response.Nu_Tipo_Transporte_Maritimo == '1')
        selected = 'selected="selected"';
      $('#oc_reservar_pedido-Nu_Tipo_Transporte_Maritimo').append('<option value="1" ' + selected + '>FCL</option>');
      
      selected = '';
      if (response.Nu_Tipo_Transporte_Maritimo == '2')//c.trading
        selected = 'selected="selected"';
      $('#oc_reservar_pedido-Nu_Tipo_Transporte_Maritimo').append('<option value="2" ' + selected + '>LCL</option>');
...
function pagoClienteOC(id, sCorrelativo){
  $( '[name="pago_cliente_oc-ID_Pedido_Cabecera"]' ).val(id);

  $( '.modal-pago_cliente_oc' ).modal('show');
  $( '#form-pago_cliente_oc' )[0].reset();
}
