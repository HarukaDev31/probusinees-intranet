var url, table_Entidad, div_chat_item='', div_items = '', iCounter=0, iCounterItems = 1;
//AUTOCOMPLETE
  ...
  */
  ...
  //chat de novedades de producto
    ...
    */
    ...
    $( '#txt-chat_producto-ID_Organizacion_item' ).val($(this).data('id_organizacion'));
    $( '#txt-chat_producto-ID_Pedido_Cabecera_item' ).val($(this).data('id_pedido_cabecera'));
    $( '#txt-chat_producto-ID_Pedido_Detalle_item' ).val($(this).data('id_pedido_detalle'));
  ...
  //chat de novedades de producto
    ...
    if ($( '[name="message_chat"]' ).val() == '' || $( '[name="message_chat"]' ).val() == null || $( '[name="message_chat"]' ).val().length==0) {
      $( '[name="message_chat"]' ).closest('.form-group').find('.help-block').html('Mensaje vacío');
      $( '[name="message_chat"]' ).closest('.form-group').removeClass('has-success').addClass('has-error');
      $( '[name="message_chat"]' ).focus();
    } else {
      ...
      for (i = 0; i < detalle.length; i++) {
        ...
        */
        ...
        "<tr id='tr_enlace_producto" + id_item + "'>"
          ...
          var class_button_chat = (parseInt(detalle[i]['Nu_Envio_Mensaje_Chat_Producto']) > 0 ? 'warning' : 'success');
          table_enlace_producto += '<button type="button" id="btn-chat_producto' + id_item + '" data-id_empresa="' + response.ID_Empresa + '" data-id_organizacion="' + response.ID_Organizacion + '" data-id_pedido_cabecera="' + response.ID_Pedido_Cabecera + '" data-id_pedido_detalle="' + id_item + '" data-nombre_producto="' + nombre_producto + '" class="mt-3 btn btn-'+class_button_chat+' btn-block btn-chat_producto"><i class="fas fa-comments"></i>&nbsp; Novedades <span class="badge bg-danger">' + detalle[i]['Nu_Envio_Mensaje_Chat_Producto'] + '</span></button>';
          table_enlace_producto += "</td>";

          table_enlace_producto += "<td class='text-left td-name' width='10%'>" + href_link + "</td>";
          table_enlace_producto += '<input type="hidden" name="addProductoTable[' + id_item + '][cantidad_oculta]" value="' + cantidad_item + '">';
          table_enlace_producto += '<input type="hidden" name="addProductoTable[' + id_item + '][nombre_comercial_oculta]" value="' + detalle[i]['Txt_Producto'] + '">';
          table_enlace_producto += '<input type="hidden" name="addProductoTable[' + id_item + '][caracteristicas_oculta]" value="' + detalle[i]['Txt_Descripcion'] + '">';
      ...
      for (i = 0; i < detalle.length; i++) {
        ...
        if(id_item_tmp != id_item){
          ...
          "<tr id='tr_enlace_producto" + id_item + "'>"
            ...
            //+ "<td class='text-left td-nota'>" + detalle[i]['Txt_Nota'] + "</td>"
            table_enlace_producto += "<td class='text-left td-contacto_proveedor'>";
              table_enlace_producto += '<input type="text" inputmode="text" id="modal-contacto_proveedor' + i + '" name="addProducto[' + i + '][contacto_proveedor]" class="mt-0 form-control required contacto_proveedor" placeholder="" maxlength="255" autocomplete="off" value="' + contacto_proveedor + '">';
            table_enlace_producto += '</td>';
            
            //+ "<td class='text-left td-nota'>" + detalle[i]['Txt_Nota'] + "</td>"
            table_enlace_producto += "<td class='text-left td-foto_proveedor'>";
              table_enlace_producto += '<input class="form-control" id="modal-foto_proveedor' + i + '" name="addProveedor[' + i + ']" type="file" accept="image/*">';
              if( imagen_proveedor != '' && imagen_proveedor != null ) {
                table_enlace_producto += "<img data-id_item='" + id_item + "' data-url_img='" + imagen_proveedor + "' src='" + imagen_proveedor + "' class='img-thumbnail img-table_item img-fluid img-resize mb-2'>";
              }
            table_enlace_producto += '</td>';
          table_enlace_producto += "</tr>";
